...
static int dvb_ca_en50221_set_configoption(struct aml_ci *ci_dev, int slot)
{
	int configoption;

	/* set the config option */
	aml_ci_bus_mem_write(ci_dev,slot,
				     config_base,
				     config_option);

	/* check it */
	configoption = aml_ci_bus_mem_read(ci_dev, slot, config_base);

	printk("Set configoption 0x%x, read configoption 0x%x\n",config_option, configoption & 0x3f);
...
static void aml_ci_bus_full_test(struct aml_ci *ci_dev)
{
		...
		int count = 1000;
		unsigned char reg;
		pr_dbg("HARD RESET START\r\n");
		aml_ci_slot_reset(ci_dev, 0);
		usleep_range(4000, 4000+100);
		pr_dbg("HARD RESET END\r\n");
		pr_dbg("READ CIS START\r\n");
		dvb_ca_en50221_parse_attributes();
		pr_dbg("READ CIS OVER\r\n");
		usleep_range(1000, 1000+100);
		dvb_ca_en50221_set_configoption(ci_dev, 0);
		pr_dbg("SW rst CAM...\r\n");
		usleep_range(1000, 1000+100);
		aml_ci_bus_io_write(ci_dev, 0,
			       COM_STA_REG, RS);
		pr_dbg("SW rst over.\r\n");
		usleep_range(dvb_ca_en50221_usleep, dvb_ca_en50221_usleep + 100);
		...
		if (1) {
			...
			while (count) {
				count--;
				reg = aml_ci_bus_io_read(
					ci_dev, 0, COM_STA_REG);
				if ((FR & reg)) {
					pr_dbg("CAM Reset error [0x%x]\r\n", reg);
					continue;
				} else {
					pr_dbg("CAM Reset Ok\r\n");
					break;
		...
		aml_ci_bus_io_write(ci_dev, 0, COM_STA_REG, IRQEN|SR);
		mdelay(1000);
		count = 100;
		while (count) {
			mdelay(1000);
			...
			if ((reg & DA)) {
				...
				pr_dbg("Buffer negotiate size date NOT avalible\r\n");
				mdelay(100);
				count--;
				continue;
			}
...
static struct class_attribute aml_ci_bus_attrs[] = {
	__ATTR(iotest,  S_IRUGO | S_IWUSR, iotest_show, iotest_store),
	__ATTR(status,  S_IRUGO | S_IWUSR, status_show, NULL),
	__ATTR(irq,     S_IRUGO | S_IWUSR, irq_show, NULL),
	__ATTR(reset,  S_IRUGO | S_IWUSR, reset_show, reset_store),
	__ATTR(pwr,  S_IRUGO | S_IWUSR, pwr_show, pwr_store),
	__ATTR(start,  S_IRUGO | S_IWUSR, start_show, start_store),
	__ATTR(wakeup,  S_IRUGO | S_IWUSR, wakeup_show, wakeup_store),
	__ATTR_NULL
};

static struct class aml_ci_bus_class = {
	.name = "aml_ci_bus_test",
	.class_attrs = aml_ci_bus_attrs,
};
...
int  aml_ci_bus_mod_init(void)
{
	...
	class_register(&aml_ci_bus_class);
	...
	if (ci_bus.raw_mode == 1) {
		...
		if (rawci_major <= 0) {
			...
			device_create(&aml_ci_bus_class, NULL,
				MKDEV(rawci_major, 0), NULL,
				RAWCI_DEV_NAME);
		}
	} else {
		pr_dbg("no register CI chrdev [%s]\n", RAWCI_DEV_NAME);
	}
