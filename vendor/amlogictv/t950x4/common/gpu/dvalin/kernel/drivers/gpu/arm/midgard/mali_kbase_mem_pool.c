...
static size_t get_gpu_total_mem(void)
{
	...
	list_for_each(entry, kbdev_list) {
		...
		list_for_each_entry(element, &kbdev->kctx_list, link) {
				total_mem += atomic_read(&(element->kctx->used_pages));
				total_mem += element->kctx->mem_pool.cur_size;
		}
...
struct page *kbase_mem_alloc_page(struct kbase_mem_pool *pool)
{
	...
	if (get_gpu_total_mem() < KBASE_MEM_THRESHOLD) {
#ifdef CONFIG_64BIT
		gfp = GFP_DMA | __GFP_ZERO | __GFP_RECLAIM;
#else
