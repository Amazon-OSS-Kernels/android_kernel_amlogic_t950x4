...
#endif
#ifdef SHINE_PROJECT
/*used for workaround HADRIAN MARIBRONZE I2C issue*/
	if(poweroff_gpioh7){
		int ret;
        	ret = xGpioSetDir(GPIOH_7,GPIO_DIR_OUT);
        	if (ret < 0) {
	...
	if (led_flag == 2) {
		...
		printf("maribronze suspend don't control led\n");
	} else {
		...
		while (xQueueReceive(xSTRQueue, &buffer, portMAX_DELAY))
		{
			switch (buffer)
			{
				...
				case WIFI_WAKEUP:
					...
					break;
				case REMOTE_CUS1_WAKEUP:
					exit_reason = REMOTE_CUS1_WAKEUP;
					break;
				case REMOTE_CUS2_WAKEUP:
					exit_reason = REMOTE_CUS2_WAKEUP;
					break;
...
void create_str_task(void)
{
	...
	ret = xInstallRemoteMessageCallbackFeedBack(AOREE_CHANNEL, MBX_CMD_SET_WOL_POWER,
					xETHPowerEnable, 0);
	if (ret == MBOX_CALL_MAX)
