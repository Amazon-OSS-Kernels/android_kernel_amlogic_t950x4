...
static struct mmc *log_mmc = NULL;
//static int log_dev = -1;
extern int can_save;
int get_log_block_size(void)
{
	return log_mmc ? log_mmc->read_bl_len : 0;
}
...
int aml_mmc_read(const char *part_name, loff_t offset, size_t len, void *buffer)
{
	...
	log_dev = find_dev_num_by_partition_name((char *)part_name);
	if (log_dev < 0) {
		printf("Cannot find dev.\n");
		return -1;
	}

	mmc = find_mmc_device(log_dev);
	if (!mmc)
		return -1;
...
//	mmc_init(log_mmc);
	...
	//read remain sz_byte bytes
	if ((n == cnt) && (sz_byte != 0)) {
		/*printf("sz_byte=%#llx bytes\n",sz_byte);*/
		addr_tmp = malloc(log_mmc->read_bl_len);
		addr_byte = (void *)(buffer + cnt * (log_mmc->read_bl_len));
		start_blk = blk + cnt;
		...
		if (log_mmc->block_dev.block_read(log_dev, start_blk, 1, addr_tmp) != 1) { // read 1 block
			free(addr_tmp);
			printf("mmc read 1 block fail\n");
			return -1;
		}
...
//	mmc_init(log_mmc);
	...
	n = log_mmc->block_dev.block_write(log_dev, blk, cnt, buffer);
	//write sz_byte bytes
	if ((n == cnt) && (sz_byte != 0)) {
		// printf("sz_byte=%#llx bytes\n",sz_byte);
		void *addr_tmp = malloc(log_mmc->write_bl_len);
		void *addr_byte = (void*)(buffer + cnt*(log_mmc->write_bl_len));
		ulong start_blk = blk+cnt;
		...
		// read 1 block
		if (log_mmc->block_dev.block_read(log_dev, start_blk, 1, addr_tmp) != 1) {
			free(addr_tmp);
			printf("mmc read 1 block fail\n");
			return -1;
		}
		...
		memcpy(addr_tmp, addr_byte, sz_byte);
		// write 1 block
		if (log_mmc->block_dev.block_write(log_dev, start_blk, 1, addr_tmp) != 1) {
			free(addr_tmp);
			printf("mmc write 1 block fail\n");
			return -1;
		}
...
int aml_log_erase(const char *log_part_name)
{
	...
	log_dev = find_dev_num_by_partition_name((char *)log_part_name);
	if (log_dev < 0) {
		printf("Cannot find wdev.\n");
		return -1;
	}
	...
	ret = amlmmc_erase_single_part(log_dev, log_mmc, (char *)log_part_name);
#endif
	argv[2]=log_part_name;
	ret = amlmmc_erase_single_part(3, argv);
