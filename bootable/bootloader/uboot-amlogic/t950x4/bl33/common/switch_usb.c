...
int android_image_update_usb_mode(const char * line)
{
    ...
    int diag_mode = 0; // 1 is diag, 0 fos
    ...
    */
    ...
    dev_flags = simple_strtoul(flags_buf, NULL, 16);
    usb_mode =((dev_flags & DEV_FLAGS_USB_DEVICE) == DEV_FLAGS_USB_DEVICE);
    diag_mode =((dev_flags & DEV_FLAGS_ENABLE_FACTORY_TEST) == DEV_FLAGS_ENABLE_FACTORY_TEST);
    ...
    if (1 == diag_mode) {
        ...
        snprintf(fat_cmd, 100, "fatload usb 0 %p FAC_BOOT_SMART.cvt", &file_pos);
        return_value=run_command(fat_cmd,0);
        if (0 == return_value) {
	    printf("smt mode detected\n");
	    usb_mode = 0;
        }
    }
