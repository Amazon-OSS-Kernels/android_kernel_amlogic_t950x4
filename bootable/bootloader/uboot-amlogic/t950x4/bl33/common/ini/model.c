...
static int handle_lcd_ext_cmd_data(struct lcd_ext_attr_s *p_attr)
{
	...
	tmp_buf = (unsigned int *)malloc(sizeof(unsigned int) * LCD_EXTERN_INIT_ON_MAX);
	if (!tmp_buf) {
		ALOGE("%s, malloc buffer memory error!!!\n", __func__);
		return -1;
	}
	...
	if (tmp_cnt > LCD_EXTERN_INIT_ON_MAX) {
		...
		for (i = 0; i < tmp_cnt; i++)
			p_attr->cmd_data[i] = (unsigned char)tmp_buf[i];
...
void model_name_convert_for_firetv_odm(char *model_name, int size)
{
	#define ITEM "/firetv_odm/"
	char model_name_temp[256] = {0};
	...
	if(strlen(model_name) <= strlen(ITEM)) {
		printf("[%s, %d] bad param\n", __FUNCTION__, __LINE__);
		return;
	}

	strcpy(model_name_temp, model_name);
	if(0 == strncmp(model_name_temp, ITEM, strlen(ITEM))) {
		memset(model_name, 0x00, size);
		sprintf(model_name, "%s%s", "/mnt/vendor", model_name_temp);
	}
...
const char *get_model_sum_path(void)
{
	...
	if (idme_get_var_external(MODEL_CONFIG_FILE_PATH, model_config_file, sizeof(model_config_file)) == 0) {
		printf("[%s, %d] get idme model config file %s\n",  __FUNCTION__, __LINE__, model_config_file);
		#ifdef CONFIG_DEVICE_PRODUCT_SHINE
		model_name_convert_for_firetv_odm(model_config_file, sizeof(model_config_file));
		printf("[%s, %d] after convert get idme model config file %s\n",  __FUNCTION__, __LINE__, model_config_file);
		#endif
