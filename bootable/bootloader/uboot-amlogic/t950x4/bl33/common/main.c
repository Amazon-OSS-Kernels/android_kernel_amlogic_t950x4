...
#if defined(UBOOT_TARGET_PRODUCT_NAME_ABC) || defined (UBOOT_TARGET_PRODUCT_NAME_ALMOND) || defined (UBOOT_TARGET_PRODUCT_NAME_ABC)
static u32 fb_width;
...
void main_loop(void)
{
	...
	if (is_transition_bootmode) {
		if (!is_transition_done) {
			ret = 0;
#if !defined(UBOOT_TARGET_PRODUCT_NAME_ALMOND) && !defined(UBOOT_TARGET_PRODUCT_NAME_ABC)
...
#if defined (UBOOT_TARGET_PRODUCT_NAME_ALMOND) && defined(UFBL_FEATURE_FASTBOOT_LOCKDOWN)
