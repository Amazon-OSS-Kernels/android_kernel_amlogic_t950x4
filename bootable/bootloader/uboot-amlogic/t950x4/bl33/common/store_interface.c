...
static int do_store_dtb_ops(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
		...
		snprintf(_cmdBuf, sizeof(_cmdBuf), "%s %s %s 0x%x", devCmd, ops, dtbLoadaddr, actualDtbSz);
		MsgP("To run cmd[%s]\n", _cmdBuf);
...
static int do_store_key_ops(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	snprintf(_cmdBuf, sizeof(_cmdBuf), "%s %s %s 0x%x", devCmd, ops, argv[3], actualDtbSz);
	MsgP("To run cmd[%s]\n", _cmdBuf);
	ret = run_command(_cmdBuf, 0);
		...
		case NAND_BOOT_FLAG:
			{
				if ((init_flag >=STORE_BOOT_ERASE_PROTECT_CACHE) && (init_flag <=STORE_BOOT_SCRUB_ALL)) {
					snprintf(str, sizeof(str), "amlnf  init  %d ",init_flag);
					run_command(str, 0);
				}

				snprintf(str, sizeof(str), "amlnf  init  %d ",1);
				printf("command:	%s <- %d\n", str, init_flag);
		...
		case EMMC_BOOT_FLAG:
			{
				...
				device_boot_flag = EMMC_BOOT_FLAG;
				snprintf(str, sizeof(str), "mmc dev %d", CONFIG_SYS_MMC_ENV_DEV);
				run_command(str,0);
				...
				if (init_flag == STORE_BOOT_ERASE_PROTECT_CACHE) { // OTA upgrade protect cache
					...
					snprintf(str, sizeof(str), "amlmmc erase %d", CONFIG_SYS_MMC_ENV_DEV);
		...
		case SPI_NAND_FLAG:
			{
				/*
				   ...
				   if ((init_flag > STORE_BOOT_ERASE_PROTECT_CACHE) && (init_flag <= STORE_BOOT_SCRUB_ALL)) {
				   snprintf(str, sizeof(str), "sf erase 0 0x%x", _SPI_FLASH_ERASE_SZ);
				   ret = run_command(str,0);
				   }
				   snprintf(str, sizeof(str), "amlnf  init  %d ",init_flag);
				   ...
				   snprintf(str, sizeof(str), "amlnf  init  %d ",4);
				   ret = run_command(str, 0);
				   }
...
static int do_store_size(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (device_boot_flag == NAND_BOOT_FLAG) {
#if defined(CONFIG_AML_NAND)
		snprintf(str, sizeof(str), "amlnf  size  %s %llx",s,addr);
		store_dbg("command:	%s", str);
		ret = run_command(str, 0);
#elif defined(CONFIG_AML_MTD)
		...
		snprintf(str, sizeof(str), "amlmmc  size  %s %llx",s,addr);
		store_dbg("command:	%s", str);
		ret = run_command(str, 0);
		if (ret != 0) {
...
static int do_store_erase(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strcmp(area, "boot") == 0) {
		...
		if (device_boot_flag == NAND_BOOT_FLAG) {
			...
			snprintf(str, sizeof(str), "sf erase  0 0x%x", CONFIG_ENV_IN_SPI_OFFSET);//store erase boot shoould NOT erase ENV in flash!
			ret = run_command(str,0);
			if (ret != 0) {
				store_msg("nand cmd %s failed",cmd);
				return -1;
			...
			snprintf(str, sizeof(str), "amlmmc  erase bootloader");
			ret = run_command(str, 0);
			if (ret != 0) {
				store_msg("amlmmc cmd %s failed",cmd);
				return -1;
			...
			for (i=0; i < erase; i++) {
				...
				snprintf(str, sizeof(str), "amlmmc switch 1 boot%d", i);
				store_dbg("command: %s\n", str);
				ret = run_command(str, 0);
				if (ret == -1) {
					//store_msg("mmc cmd %s failed \n",cmd);
...
E_SWITCH_BACK:
			...
			snprintf(str, sizeof(str), "amlmmc switch 1 user");
			store_dbg("command: %s\n", str);
			ret = run_command(str, 0);
			if (ret != 0) {
				store_msg("amlmmc cmd %s failed \n",cmd);
		...
		if (device_boot_flag == EMMC_BOOT_FLAG) {
			snprintf(str, sizeof(str), "emmc erase key");
			ret = run_command(str, 0);
			if (ret != 0) {
		...
		#if defined(CONFIG_AML_NAND) || defined(CONFIG_AML_MTD)
			snprintf(str, sizeof(str), "amlnf key_erase");
			ret = run_command(str, 0);
			if (ret != 0) {
				store_msg("emmc cmd %s failed",cmd);
				return CMD_RET_USAGE;
		...
		if (device_boot_flag == EMMC_BOOT_FLAG) {
			snprintf(str, sizeof(str), "emmc erase dtb");
			ret = run_command(str, 0);
			if (ret != 0) {
				store_msg("emmc cmd %s failed",cmd);
				return CMD_RET_USAGE;
		...
		#if defined(CONFIG_AML_NAND) || defined(CONFIG_AML_MTD)
			snprintf(str, sizeof(str), "amlnf dtb_erase");
			ret = run_command(str, 0);
			if (ret != 0) {
				store_msg("emmc cmd %s failed",cmd);
				return CMD_RET_USAGE;
		...
		if (device_boot_flag == EMMC_BOOT_FLAG) {
			...
			snprintf(str, sizeof(str), "nand erase.part %s", argv[3]);
...
static int do_store_scrub(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	snprintf(str, sizeof(str), "amlnf  scrub %d", (int)off);
	if (device_boot_flag == NAND_BOOT_FLAG) {
		#if defined(CONFIG_AML_NAND)
		ret = run_command(str, 0);
		#elif defined(CONFIG_AML_MTD)
...
static int do_store_rom_protect(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (device_boot_flag == NAND_BOOT_FLAG) {
#if defined(CONFIG_AML_NAND)
		snprintf(str, sizeof(str),"amlnf  rom_protect  %s", area);
		store_dbg("command:	%s", str);
...
static int do_store_rom_write(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (device_boot_flag == NAND_BOOT_FLAG) {
		...
		snprintf(str, sizeof(str), "amlnf rom_write 0x%llx 0x%llx 0x%llx", addr, off, size);
		store_dbg("command:	%s", str);
		ret = run_command(str, 0);
#else
/*
...
*/
		...
		for (i = 0; i < Bl2CpyNum; ++i)
		{
			...
			snprintf(str, sizeof(str), "amlnf rom_erase %d", i);
			ret = run_command(str, 0);
			if (ret) {
			...
			//copyOff = i * Bl2Size;
			snprintf(str, sizeof(str), "amlnf bl2_write 0x%llx %d 0x%x", addr, i, Bl2Size);
			debugP("runCmd[%s]\n", str);
			ret = run_command(str, 0);
			if (ret) {
		...
		for ( i = 0; i < tplCpyNum && updateTpl; ++i )
		{
			...
			snprintf(str, sizeof(str), "amlnf fip_erase %d", i);
			ret = run_command(str, 0);
			if (ret) {
			...
			snprintf(str, sizeof(str), "amlnf fip_write 0x%llx %llx 0x%x", addr, copyOff, tplWriteSz);
			debugP("runCmd[%s]\n", str);
			ret = run_command(str, 0);
			if (ret) {
				ErrP("Failed at pgram bl2[%d],ret=%d\n", i, ret);
		...
		snprintf(str, sizeof(str), "sf  erase  0x%llx  0x%llx ", off, size);
		ret = run_command(str, 0);
		if (ret != 0) {
			store_msg("nand cmd %s failed",cmd);
			return -1;
		}
		snprintf(str, sizeof(str), "sf  write 0x%llx  0x%llx  0x%llx ",addr, off, size);
		store_dbg("command:	%s", str);
		ret = run_command(str, 0);
		if (ret != 0) {
			store_msg("nand cmd %s failed",cmd);
		...
		snprintf(str, sizeof(str), "amlmmc  write bootloader 0x%llx  0x%llx  0x%llx", addr, off, size);
		store_dbg("command: %s\n", str);
		ret = run_command(str, 0);
		if (ret != 0) {
			store_msg("amlmmc cmd %s failed \n",cmd);
...
#if defined(CONFIG_AML_NAND) || defined(CONFIG_AML_MTD)
#ifndef CONFIG_DISCRETE_BOOTLOADER
		snprintf(str, sizeof(str), "amlnf rom_read 0x%llx 0x%llx 0x%llx", addr, off, size);
		store_dbg("command:	%s", str);
		ret = run_command(str, 0);
#else
/*
...
*/
		...
		for (i = 0; i < Bl2CpyNum; ++i)
		{
			...
			snprintf(str, sizeof(str), "amlnf bl2_read 0x%p %x 0x%x", readBuf, i, Bl2Size);
			debugP("runCmd[%s]\n", str);
			ret = run_command(str, 0);
			if (ret) {
				ErrP("Failed at pgram bl2[%d],ret=%d\n", i, ret);
		...
		if (tplRealSz > 0) // to support dump only bl2
		{
			...
			for ( i = 0; i < tplCpyNum && !ret; ++i )
			{
				...
				copyOff = i * tplCapSize;
				snprintf(str, sizeof(str), "amlnf fip_read 0x%p %llx 0x%x", readBuf, copyOff, tplRealSz);
				debugP("runCmd[%s]\n", str);
				ret = run_command(str, 0);
				if (ret) {
					ErrP("Failed at pgram bl2[%d],ret=%d\n", i, ret);
		...
		snprintf(str, sizeof(str), "sf  read 0x%llx  0x%llx  0x%llx ",addr, off, size);
		store_dbg("command:	%s", str);
		ret = run_command(str, 0);
		if (ret != 0) {
			store_msg("nand cmd %s failed",cmd);
	   ...
	   if ( cpu_id.family_id >= MESON_CPU_MAJOR_ID_GXL)
				off += 512;
		store_dbg("MMC BOOT, %s %d \n",__func__,__LINE__);
		snprintf(str, sizeof(str),"amlmmc  read bootloader 0x%llx  0x%llx  0x%llx", addr, off, size);
		store_dbg("command: %s\n", str);
...
static int do_store_read(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
...
#if defined(CONFIG_AML_NAND) || defined(CONFIG_AML_MTD)
#if defined(CONFIG_AML_NAND)
	snprintf(str, sizeof(str), "amlnf  read_byte %s 0x%llx  0x%llx  0x%llx",s, addr, off, size);
#elif defined(CONFIG_AML_MTD)
	#if  defined(CONFIG_DISCRETE_BOOTLOADER)
	if ( !strcmp(CONFIG_TPL_PART_NAME, s) ) {
		...
		snprintf(str, sizeof(str), "amlnf fip_read 0x%llx %llx 0x%llx", addr, copyOff, size);
	} else
	#endif // #if  defined(CONFIG_DISCRETE_BOOTLOADER)
	{
		...
		snprintf(str, sizeof(str), "nand  read %s 0x%llx  0x%llx  0x%llx",s, addr, off, size);
	}
#endif // #if defined(CONFIG_AML_NAND)
#else
		ret = -1;
		...
		store_dbg("spi+mmc , %s %d ",__func__,__LINE__);
		snprintf(str, sizeof(str), "amlmmc  read %s 0x%llx  0x%llx  0x%llx", s, addr, off, size);
		store_dbg("command:	%s\n", str);
		ret = run_command(str, 0);
		if (ret != 0) {
			store_msg("amlmmc cmd %s failed \n",cmd);
...
static int do_store_write(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (device_boot_flag == NAND_BOOT_FLAG) {
#if defined(CONFIG_AML_NAND) || defined(CONFIG_AML_MTD)
	#if defined(CONFIG_AML_NAND)
		snprintf(str, sizeof(str), "amlnf write_byte %s 0x%llx  0x%llx  0x%llx", s, addr, off, size);
		ret = run_command(str, 0);
	#elif defined(CONFIG_AML_MTD)
		#if  defined(CONFIG_DISCRETE_BOOTLOADER)
		if ( !strcmp(CONFIG_TPL_PART_NAME, s) ) {
			...
			for ( i = 0; i < tplCpyNum; ++i )
			{
				...
				snprintf(str, sizeof(str), "amlnf fip_write 0x%llx %llx 0x%llx", addr, copyOff, size);
				debugP("runCmd[%s]\n", str);
				ret = run_command(str, 0);
				if (ret) {
					ErrP("Failed at pgram bl2[%d],ret=%d\n", i, ret);
		...
		#endif // #if  defined(CONFIG_DISCRETE_BOOTLOADER)
		{
			...
			snprintf(str, sizeof(str), "nand write %s 0x%llx  0x%llx  0x%llx",s, addr, off, size);
			ret = run_command(str, 0);
		}
   #endif
#else
		...
		store_dbg("spi+mmc , %s %d ",__func__,__LINE__);
		snprintf(str, sizeof(str), "amlmmc  write %s 0x%llx  0x%llx  0x%llx", s, addr, off, size);
		store_dbg("command: %s\n", str);
		ret = run_command(str, 0);
		if (ret != 0) {
			store_msg("amlmmc cmd %s failed \n",cmd);
