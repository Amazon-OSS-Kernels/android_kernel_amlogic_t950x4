...
int Need_power_off_GPIOH7(void)
{
	...
	if((strstr(config_name,"maribronze") != NULL) && (board_id == 5) && (strstr(model_name, "HVT_FHD_32_1_T") != NULL ||
...
static int set_kenrel_buad_rate(int buad_rate_index)
{
        switch (buad_rate_index) {
        case 115200:
        //      run_command("setenv baudrate 115200" , 0);
                run_command("setenv initargs console=ttyS0,115200 ${initargs}", 0);
                break;
        case 460800:
                run_command("setenv initargs console=ttyS0,460800 ${initargs}", 0);
                break;
        case 921600:
                run_command("setenv initargs console=ttyS0,921600 ${initargs}", 0);
                break;
        default:
                printf("not support the buad rate:%d\n", buad_rate_index);
                break;
        }
        return 0;
}

void set_mail_box_data(void)
{
	uint32_t model_name_data[10];
	int ret = -1;
	const char *wol_gpio_value = NULL;
	const char *led_flag_value = NULL;
	const char *led_brightness_value = NULL;
	const char *buad_rate= NULL;
	int buad_rate_index = 0;
	char config_name[64] = {0};
	...
	wol_gpio_value = IniGetString("DEFAULT", "WOL_GPIO", "null");
	...
	printf("set_LED_brightness:%d\n",model_name_data[2]);
	printf("power of GPIOH_7:%d\n",Need_power_off_GPIOH7());
	printf("set buad_rate:%d\n",buad_rate_index);
	set_kenrel_buad_rate(buad_rate_index);
...
*/
		...
		case 0x60000000:
			...
			if (strcmp("shine", product_name) == 0  && strstr(config_name,"maribronze") == NULL && strstr(config_name,"abc123") == NULL) {
				if ( get_amp_name( amp_name, sizeof(amp_name)) == 0 ) {
					if ( strcmp("acm8625", amp_name ) == 0 )
						strcpy(dtb_name, "t5d_t950d4_shine-hw-v4-1.5g\0");
					else
						strcpy(dtb_name, "t5d_t950d4_shine-1.5g\0");
				} else
					strcpy(dtb_name, "t5d_t950d4_shine-1.5g\0");
			} else if (strstr(config_name,"maribronze") != NULL) {
                                strcpy(dtb_name, "t5d_t950d4_maribronze-1.5g\0");
			} else if (strstr(config_name,"abc123") != NULL) {
                                strcpy(dtb_name, "t5d_t950d4_abc123-1.5g\0");
			} else
