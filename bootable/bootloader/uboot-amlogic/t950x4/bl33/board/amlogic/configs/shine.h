...
#define UBOOT_TARGET_PRODUCT_NAME_SHINE
...
// this code already no use, will define the power key in bl30 
/*
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"led_flag_value=none\0" \
        ...
        "firetv_odm=0\0"\
        "initargs="\
            "init=/init console=ttyS0,460800 no_console_suspend earlycon=aml-uart,0xffd23000 printk.devkmsg=on ramoops.pstore_en=1 ramoops.record_size=0x8000 ramoops.console_size=0x4000 "\
            "\0"\
        "upgrade_check="\
        ...
        "storeargs="\
            ...
            "setenv bootargs ${bootargs} androidboot.boardinfo=${boardinfo};"\
            ...
            "setenv bootargs ${bootargs} androidboot.firetv_odm=${firetv_odm}; "\
	    "setenv bootargs ${bootargs} androidboot.led_flag=${led_flag_value}; "\
            "run cmdline_keys;"\
            "\0"\
        "cec_init="\
        ...
        "cold_boot_normal_check="\
            ...
            "if test ${powermode} = on; then "\
                /*"run try_auto_burn; "*/\
            "else if test ${powermode} = standby && itest ${bypass_standby} != 1; then "\
                "run cec_init;"\
                "systemoff; "\
            "else if test ${powermode} = last; then "\
                ...
                "if test ${suspend} = off; then "\
                    /*"run try_auto_burn; "*/\
                "else if test ${suspend} = on && itest ${bypass_standby} != 1; then "\
                    "run cec_init;"\
                    "systemoff; "\
                "else if test ${suspend} = shutdown && itest ${bypass_standby} != 1; then "\
                    "run cec_init;"\
                    "systemoff; "\
                "fi; fi; fi; "\
            "fi; fi; fi; "\
        ...
        "cmdline_keys="\
            "if keyman init 0x1234; then "\
                "if keyman read usid ${loadaddr} str; then "\
                    ...
                    "setenv serial ${usid};"\
                "else "\
                    "setenv bootargs ${bootargs} androidboot.serialno=123456789ABCDEFG;"\
...
#define CONFIG_SAVE_BL33_LOG
#ifdef CONFIG_SAVE_BL33_LOG
#define CONFIG_PRE_BL33_LOG_ADDR		(0x04e00000)
#define PRE_LOG_SIZE				(8 * 1024)
#endif
...
#define CONFIG_CMD_USB 1
	...
	#define CONFIG_USB_GPIO_PWR  			GPIOEE(GPIOH_11)
	#define CONFIG_USB_GPIO_PWR_NAME		"GPIOH_11"
...
#define CONFIG_DEVICE_PRODUCT	"shine"
...
#define CONFIG_MD5SUM_VERIFY_PARTITION
