...
/*clear tuning para on emmc, the offset is 0x14400*/
static int aml_clear_tuning_para(struct mmc *mmc)
{
	unsigned int size;
	unsigned int *buf;
	int para_size;
	...
	printf("hs400 parameter is cleared\n");
	free(buf);
	return 0;
}
...
_err:
	aml_clear_tuning_para(mmc);
	aml_write_chip_id(mmc);
	return 0;
}
