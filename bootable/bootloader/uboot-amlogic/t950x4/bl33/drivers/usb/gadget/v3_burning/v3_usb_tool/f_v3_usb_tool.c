...
{
	memcpy(response_str, "FAIL", AML_DNL_CMD_ACK_LEN + 1); //+1 '\0'
	if (s)strncat(response_str, s, RESPONSE_LEN - AML_DNL_CMD_ACK_LEN - 1) ;
}

static void fastboot_okay(const char *s)
{
	memcpy(response_str, "OKAY", AML_DNL_CMD_ACK_LEN + 1); //+1 is '\0'
	if (s)strncat(response_str, s, RESPONSE_LEN - AML_DNL_CMD_ACK_LEN - 1) ;
}
