...
void set_bl_status_reg(int status)
{
	uint32_t val = 0;
...
static int aml_bl_config_load_from_dts(char *dt_addr, unsigned int index, struct bl_config_s *bconf)
{
	...
	if (propdata == NULL) {
		...
		sprintf(bconf->name, "backlight_%d", index);
            ...
            strncpy(bconf->name, propdata, sizeof(bconf->name) - 1);
	}
...
static int aml_bl_init_load_from_dts(char *dt_addr, struct bl_config_s *bconf)
{
	...
	if (propdata == NULL) {
		...
		while (i < BL_GPIO_NUM_MAX) {
			...
			if (strlen(str) == 0)
				break;
			strncpy(bconf->gpio_name[i], str, sizeof(bconf->gpio_name[i]) - 1);
			if (lcd_debug_print_flag)
				LCDPR("bl: i=%d, gpio=%s\n", i,
				      bconf->gpio_name[i]);
			i++;
