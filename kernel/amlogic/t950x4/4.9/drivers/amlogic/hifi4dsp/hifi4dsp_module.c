...
int get_dsp_online_status(const char *dsp_name)
{
	...
	if (!strcmp(dsp_name, "dsp_dev"))
		dspid = 0;
	else if (!strcmp(dsp_name, "dspb_dev"))
		dspid = 1;
	else
		return 1;
	return hifi4dsp_p[dspid]->dsp->dspstatus;
}
			     ...
			     const char *buf, size_t size)
{
	...
	if (!strncmp(buf, "hifi4a", 6)) {
		pr_debug("AP send hifi4a suspend message...\n");
		scpi_send_data(message, sizeof(message),
				  SCPI_DSPA, SCPI_CMD_HIFI4SUSPEND,
				  message, sizeof(message));
		/*swith dsp clk to normal*/
		hifi4dsp_clk_to_24M(hifi4dsp_p[0]->dsp);
	} else if (!strncmp(buf, "hifi4b", 6)) {
		pr_debug("AP send hifi4b suspend message...\n");
		scpi_send_data(message, sizeof(message),
				  SCPI_DSPB, SCPI_CMD_HIFI4SUSPEND,
				  message, sizeof(message));
		/*swith dsp clk to normal*/
		hifi4dsp_clk_to_24M(hifi4dsp_p[1]->dsp);
	} else {
		pr_info("please input the right args : hifi4a/hifi4b\n");
	}
			    ...
			    const char *buf, size_t size)
{
	...
	if (!strncmp(buf, "hifi4a", 6)) {
		/*swith dsp clk to normal*/
		hifi4dsp_clk_to_normal(hifi4dsp_p[0]->dsp);
		pr_debug("AP send hifi4a resume message...\n");
		scpi_send_data(message, sizeof(message),
				  SCPI_DSPA, SCPI_CMD_HIFI4RESUME,
				  message, sizeof(message));
		...
		/*swith dsp clk to normal*/
		hifi4dsp_clk_to_normal(hifi4dsp_p[1]->dsp);
		pr_debug("AP send hifi4b resume message...\n");
		scpi_send_data(message, sizeof(message),
				  SCPI_DSPB, SCPI_CMD_HIFI4RESUME,
				  message, sizeof(message));
	} else {
...
void suspend_create_file(struct device *dev)
{
	device_create_file(dev, &dev_attr_suspend);
}

void resume_create_file(struct device *dev)
{
