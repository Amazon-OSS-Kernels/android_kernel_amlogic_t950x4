...
static int bt_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		BT_ERR("request_irq error ret=%d", ret);

	disable_irq(pdata->irqno_wakeup);

	ret = device_init_wakeup(&pdev->dev, 1);
	if (ret)
		BT_ERR("device_init_wakeup failed: %d", ret);
	/*Wake up the interrupt*/
	ret = dev_pm_set_wake_irq(&pdev->dev, pdata->irqno_wakeup);
	if (ret)
		BT_ERR("dev_pm_set_wake_irq failed: %d", ret);
	//register input
	input_dev = input_allocate_device();
	if (!input_dev) {
		BT_ERR("input_allocate_device failed: %d", ret);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		BT_ERR("input_register_device failed: %d", ret);
