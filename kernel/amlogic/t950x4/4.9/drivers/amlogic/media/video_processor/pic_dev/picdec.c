...
static void *aml_mm_vmap(phys_addr_t phys, unsigned long size)
{
	...
	vfree(pages);
	aml_pr_info(1, "[HIGH-MEM-MAP] pa(%lx) to va(%p), size: %d\n",
...
static int picdec_start(void)
{
	...
	if (picdec_device.use_reserved) {
		...
		if (!cma_layout_flag)
			picdec_device.vir_addr = aml_map_phyaddr_to_virt
						(map_start,
						map_size);
	}
...
int picdec_pre_process(void)
{
	...
	if (picdec_device.use_reserved) {
		...
		buffer_start = aml_map_phyaddr_to_virt
