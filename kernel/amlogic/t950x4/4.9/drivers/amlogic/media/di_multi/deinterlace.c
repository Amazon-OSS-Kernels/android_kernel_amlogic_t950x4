...
static void post_ready_buf_set(unsigned int ch, struct di_buf_s *di_buf)
{
	...
	if (di_buf->local_ud &&
		...
		vframe_ret->vf_ud_param.ud_param.pbuf_addr =
		    (void *)di_buf->local_ud;
		vframe_ret->vf_ud_param.ud_param.buf_len =
		    di_buf->local_ud_used_size;
	} else {
