...
/*
 * rx_set_hdcp14_unify_secure_key
 */
unsigned int rx_set_hdcp14_unify_secure_key(void)
{
	struct arm_smccc_res res;
	unsigned int ret;

	arm_smccc_smc(HDCP14_SET_SEC_KEY, 0, 0, 0, 0, 0, 0, 0, &res);
	ret = (unsigned int)((res.a0)&0xffffffff);
	if (log_level & HDCP_LOG)
		rx_pr("set unify secure-%d", ret);
	return ret;
}
...
void rx_hdcp14_config(const struct hdmi_rx_hdcp *hdcp)
{
	...
	if (hdcp14_key_mode == SECURE_MODE || hdcp_tee_path) {
		rx_set_hdcp14_secure_key();
		rx_pr("hdcp1.4 provision mode\n");
	} else {
		if (!rx_set_hdcp14_unify_secure_key())
...
void rx_ddc_active_monitor(void)
{
	...
	temp = temp & 0xff;
	if (temp < 0x3f && temp != 8 && temp != 0xc && temp && temp != 1 && temp != 0x14) {
