	...
	const struct vinfo_s *vinfo)
{
	...
	} else if (in_p.video_ar.numerator * 3 ==
		in_p.video_ar.denominator * 4) {
	...
	if (is_ud_param_valid(vf->vf_ud_param)) {
		...
		in_p.ud_param_size =
			vf->vf_ud_param.ud_param.buf_len; /* data_size? */
	}
	...
	if (ret >= 0) {
		...
		memcpy(&layer_info->afd_pos, &out_p.dst_pos,
			sizeof(struct pos_rect_s));
		memcpy(&layer_info->afd_crop, &out_p.crop_info,
