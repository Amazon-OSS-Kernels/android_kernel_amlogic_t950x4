...
void qam_auto_scan(int auto_qam_enable)
{
	...
	if (auto_qam_enable) {
		if (devp->auto_sr)
			dvbc_cfg_sr_scan_speed(SYM_SPEED_MIDDLE);
		else
			dvbc_cfg_sr_scan_speed(SYM_SPEED_NORMAL);

		/* j83b */
		if (devp->atsc_mode == QAM_64 || devp->atsc_mode == QAM_256 ||
			devp->atsc_mode == QAM_AUTO ||
			((devp->data->hw_ver != DTVDEMOD_HW_T5) && !is_meson_t5d_cpu())) {
			dvbc_cfg_tim_sweep_range(SYM_SPEED_NORMAL);
		} else {
			if (devp->auto_sr)
				dvbc_cfg_tim_sweep_range(SYM_SPEED_MIDDLE);
			else
				dvbc_cfg_tim_sweep_range(SYM_SPEED_NORMAL);
		}
...
void demod_dvbc_set_qam(enum qam_md_e qam)
{
	...
	PR_DVBC("%s last_qam_mode %d, qam %d.\n",
			__func__, devp->last_qam_mode, qam);

	if (devp->last_qam_mode != QAM_MODE_NUM &&
			devp->last_qam_mode != qam) {
		/* restore register value, switching between different qam. */
		switch (devp->last_qam_mode) {
		case QAM_MODE_16:
			qam_write_reg(0x71, reg_value[0]);
			qam_write_reg(0x72, reg_value[1]);
			qam_write_reg(0x73, reg_value[2]);
			qam_write_reg(0x75, reg_value[3]);
			qam_write_reg(0x76, reg_value[4]);
			qam_write_reg(0x7a, reg_value[5]);
			qam_write_reg(0x93, reg_value[6]);
			qam_write_reg(0x94, reg_value[7]);
			break;
		...
		case QAM_MODE_32:
			...
			qam_write_reg(0x75, reg_value[3]);
			qam_write_reg(0x76, reg_value[4]);
			qam_write_reg(0x77, reg_value[5]);
			qam_write_reg(0x7a, reg_value[6]);
			qam_write_reg(0x7c, reg_value[7]);
			qam_write_reg(0x7d, reg_value[8]);
			qam_write_reg(0x7e, reg_value[9]);
			qam_write_reg(0x93, reg_value[10]);
			qam_write_reg(0x94, reg_value[11]);
			break;

		case QAM_MODE_64:
			if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
				qam_write_reg(0x9c, reg_value[0]);
				qam_write_reg(0x57, reg_value[1]);
			}
			break;
		...
		case QAM_MODE_128:
			...
			qam_write_reg(0x76, reg_value[4]);
			qam_write_reg(0x77, reg_value[5]);
			qam_write_reg(0x78, reg_value[6]);
			qam_write_reg(0x7a, reg_value[7]);
			qam_write_reg(0x7c, reg_value[8]);
			qam_write_reg(0x7d, reg_value[9]);
			qam_write_reg(0x7e, reg_value[10]);
			qam_write_reg(0x93, reg_value[11]);
			qam_write_reg(0x94, reg_value[12]);
			break;
	...
	if (devp->auto_sr)
		qam_write_reg(QAM_MODE_CFG,
			(qam_read_reg(QAM_MODE_CFG) & ~7)
				| (qam & 7) | (1 << 17));
	else
		qam_write_reg(QAM_MODE_CFG,
			(qam_read_reg(QAM_MODE_CFG) & ~7) | (qam & 7));
	...
	switch (qam) {
	case QAM_MODE_16:
		reg_value[0] = qam_read_reg(0x71);
		reg_value[1] = qam_read_reg(0x72);
		reg_value[2] = qam_read_reg(0x73);
		reg_value[3] = qam_read_reg(0x75);
		reg_value[4] = qam_read_reg(0x76);
		reg_value[5] = qam_read_reg(0x7a);
		reg_value[6] = qam_read_reg(0x93);
		reg_value[7] = qam_read_reg(0x94);
	...
	case QAM_MODE_32:
		...
		reg_value[3] = qam_read_reg(0x75);
		reg_value[4] = qam_read_reg(0x76);
		reg_value[5] = qam_read_reg(0x77);
		reg_value[6] = qam_read_reg(0x7a);
		reg_value[7] = qam_read_reg(0x7c);
		reg_value[8] = qam_read_reg(0x7d);
		reg_value[9] = qam_read_reg(0x7e);
		reg_value[10] = qam_read_reg(0x93);
		reg_value[11] = qam_read_reg(0x94);
	...
	case QAM_MODE_64:
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			reg_value[0] = qam_read_reg(0x9c);
			reg_value[1] = qam_read_reg(0x57);
	...
	case QAM_MODE_128:
		...
		reg_value[4] = qam_read_reg(0x76);
		reg_value[5] = qam_read_reg(0x77);
		reg_value[6] = qam_read_reg(0x78);
		reg_value[7] = qam_read_reg(0x7a);
		reg_value[8] = qam_read_reg(0x7c);
		reg_value[9] = qam_read_reg(0x7d);
		reg_value[10] = qam_read_reg(0x7e);
		reg_value[11] = qam_read_reg(0x93);
		reg_value[12] = qam_read_reg(0x94);
...
void dvbc_reg_initial(struct aml_demod_sta *demod_sta, struct dvb_frontend *fe)
{
	u32 clk_freq;
	u32 adc_freq;
	/*ary no use u8 tuner;*/
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
		...
		if (tuner_find_by_name(fe, "r842") ||
			...
			if (devp->atsc_mode == QAM_64 || devp->atsc_mode == QAM_256 ||
				devp->atsc_mode == QAM_AUTO)
				qam_write_reg(0x25,
					(qam_read_reg(0x25) & 0xFFFFFFF0) | j83b_agc_target);
			else
				qam_write_reg(0x25,
					(qam_read_reg(0x25) & 0xFFFFFFF0) | dvbc_agc_target);
		}
	}
	...
	if (devp->auto_sr)
		dvbc_cfg_sr_cnt(SYM_SPEED_MIDDLE);
	else
...
/* configure symbol rate step step 0*/
	...
	/* support CI+ card */
	if (demod_sta->ci_mode == 1)
		qam_write_bits(0x11, 0x0, 24, 8);//change 0x98 to 0x90
	else
		...
		struct aml_demod_sts *demod_sts, struct seq_file *seq)
{
	...
	demod_sts->dat1 = tuner_get_ch_power(&devp->frontend);
...
void dvbc_init_reg_ext(void)
{
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
		...
		qam_write_reg(0x12, 0x50bb800);//0x50e1000 -->0x50bb800
		qam_write_reg(0x30, 0x41f2f69);
	}
	...
	if (tuner_find_by_name(fe, "r842") ||
		tuner_find_by_name(fe, "r836") ||
		tuner_find_by_name(fe, "r850")) {
		qam_write_reg(0x7, 0xf13);

		/* agc slow */
		qam_write_reg(0x3d, 0xf22);
		qam_write_reg(0x30, 0x41fff69);
	} else {
		qam_write_reg(0x7, 0xf33);

		/* default value. */
		qam_write_reg(0x3d, 0xf24);
	}
}
...
void dvbc_cfg_sr_cnt(enum dvbc_sym_speed spd)
{
	switch (spd) {
	case SYM_SPEED_HIGH:
...
void dvbc_cfg_sr_scan_speed(enum dvbc_sym_speed spd)
{
	switch (spd) {
	case SYM_SPEED_MIDDLE:
		qam_write_reg(SR_SCAN_SPEED, 0x245bf45c);
		break;
	case SYM_SPEED_HIGH:
...
void dvbc_cfg_tim_sweep_range(enum dvbc_sym_speed spd)
{
	switch (spd) {
	case SYM_SPEED_MIDDLE:
		qam_write_reg(TIM_SWEEP_RANGE_CFG, 0x220000);
		break;
	case SYM_SPEED_HIGH:
...
void dvbc_cfg_sw_hw_sr_max(unsigned int max_sr)
{
	/* bit[0-15]: sw_symbol_rate. */
	qam_write_bits(0xd, max_sr & 0xffff, 0, 16);

	/* bit[8-23]: hw_symbol_rate_max. */
	qam_write_bits(0x11, max_sr & 0xffff, 8, 16);
}
