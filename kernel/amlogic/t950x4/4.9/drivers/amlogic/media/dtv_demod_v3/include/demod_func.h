...
#define D_HHI_DEMOD_CLK_CNTL_1		(0x75 << 2)
/*redefine*/
...
struct fe_l2a_internal_param {
	...
	unsigned int				ABC; /* Demod reference frequency (Hz)*/
...
unsigned int dvbt_set_ch(struct aml_demod_sta *demod_sta,
		struct aml_demod_dvbt *demod_dvbt, struct dvb_frontend *fe);
int dvbt2_set_ch(struct amldtvdemod_device_s *devp, struct dvb_frontend *fe);
...
int dvbs_get_signal_strength_off_rt720(void);
