		  ...
		  struct aml_demod_sys *demod_sys)
{
	...
	clocks_set_sys_defaults(&devp->frontend, demod_sta, clk_adc);
	/* set adc clk */
...
{
	demod_set_demod_reg(val, dtv_afe_reg_base + reg);

	return 0;
}
unsigned int dd_tvafe_reg_read(unsigned int addr)
{
	return demod_read_demod_reg(dtv_afe_reg_base + addr);
}
#endif
