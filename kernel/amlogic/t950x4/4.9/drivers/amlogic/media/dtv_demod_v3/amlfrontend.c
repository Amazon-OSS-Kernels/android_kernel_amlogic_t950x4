...
static int amdemod_stat_islock(enum fe_delivery_system delsys)
{
	...
	case SYS_DVBC_ANNEX_B:
		if ((devp->atsc_mode == QAM_64) || (devp->atsc_mode == QAM_256)) {
			if ((atsc_read_iqr_reg() >> 16) == 0x1f)
				ret = 1;
		} else if (devp->atsc_mode == VSB_8) {
...
void store_dvbc_qam_mode(int qam_mode, int symbolrate)
{
	...
	devp->para_demod.dvbc_qam = qam_para;
	devp->para_demod.dvbc_symbol = symbolrate;
}
...
int timer_set_max(struct amldtvdemod_device_s *devp,
		enum ddemod_timer_s tmid, unsigned int max_val)
{
	devp->gtimer[tmid].max = max_val;

	return 0;
}

int timer_begain(struct amldtvdemod_device_s *devp, enum ddemod_timer_s tmid)
{
	devp->gtimer[tmid].start = jiffies_to_msecs(jiffies);
	devp->gtimer[tmid].enable = 1;

	PR_TIME("st %d=%d\n", tmid, (int)devp->gtimer[tmid].start);
	return 0;
}

int timer_disable(struct amldtvdemod_device_s *devp, enum ddemod_timer_s tmid)
{
	devp->gtimer[tmid].enable = 0;

	return 0;
}

int timer_is_en(struct amldtvdemod_device_s *devp, enum ddemod_timer_s tmid)
{
	return devp->gtimer[tmid].enable;
}

int timer_not_enough(struct amldtvdemod_device_s *devp, enum ddemod_timer_s tmid)
{
	int ret = 0;
	unsigned int time;

	if (devp->gtimer[tmid].enable) {
		time = jiffies_to_msecs(jiffies);
		if ((time - devp->gtimer[tmid].start) < devp->gtimer[tmid].max)
			ret = 1;
	}
	return ret;
}

int timer_is_enough(struct amldtvdemod_device_s *devp, enum ddemod_timer_s tmid)
{
	int ret = 0;
	unsigned int time;
...
int timer_tuner_not_enough(struct amldtvdemod_device_s *devp)
{
	int ret = 0;
	unsigned int time;
	...
	if (devp->gtimer[tmid].enable) {
		time = jiffies_to_msecs(jiffies);
		if ((time - devp->gtimer[tmid].start) < 200) {
			PR_TIME("nowt=%d\n", (int)time);
			ret = 1;
		}
	}
...
unsigned int demod_get_adc_clk(void)
{
	...
	return devp->demod_status.adc_freq;
}
...
unsigned int demod_get_sys_clk(void)
{
	...
	return devp->demod_status.clk_freq;
}
	...
	(struct dvb_frontend *fe, enum fe_status *status)
{
	...
	/*check tuner*/
	if (!timer_tuner_not_enough(devp)) {
		...
		if (tuner_find_by_name(fe, "r842")) {
			...
			if (strength <= -80)
				strength = dvbc_get_power_strength(qam_read_reg(0x27) & 0x7ff,
						strength);
		}
	...
	if (demod_sts.ch_sts & 0x1) {
		...
		*status =
		...
		if (timer_not_enough(devp, D_TIMER_DETECT)) {
			*status = 0;
			PR_TIME("s=0\n");
		} else {
			*status = FE_TIMEDOUT;
	...
	if (devp->last_lock != ilock) {
		PR_DBG("%s: %s.\n", __func__,
			 ilock ? "!!  >> LOCK << !!" : "!! >> UNLOCK << !!");
		devp->last_lock = ilock;
		if (ilock == 0)
...
static int demod_dvbc_speed_up(struct dvb_frontend *fe, enum fe_status *status)
{
	struct amldtvdemod_device_s *devp = (struct amldtvdemod_device_s *)fe->demodulator_priv;
...
{
	*snr = (qam_read_reg(0x5) & 0xfff) * 10 / 32;

	PR_DVBC("demod snr is %d.%d\n", *snr / 10, *snr % 10);
...
static int Gxtv_Demod_Dvbc_Init(int mode)
{
	...
	memset(&devp->demod_status, 0, sizeof(devp->demod_status));

	devp->demod_status.delsys = SYS_DVBC_ANNEX_A;

	if (mode == ADC_MODE) {
		sys.adc_clk = ADC_CLK_25M;
		sys.demod_clk = DEMOD_CLK_200M;
		devp->demod_status.tmp = ADC_MODE;
	} else {
		sys.adc_clk = ADC_CLK_24M;
		sys.demod_clk = DEMOD_CLK_72M;
		devp->demod_status.tmp = CRY_MODE;
	}
	...
	devp->demod_status.ch_if = DEMOD_5M_IF;
	devp->auto_flags_trig = 0;
	devp->demod_status.adc_freq = sys.adc_clk;
	devp->demod_status.clk_freq = sys.demod_clk;

	PR_DBG("[%s] adc_clk is %d, demod_clk is %d, Pll_Mode is %d\n",
			__func__, sys.adc_clk, sys.demod_clk, devp->demod_status.tmp);

	/* sys clk div */
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1))
		dd_tvafe_hiu_reg_write(D_HHI_DEMOD_CLK_CNTL, 0x502);

	demod_set_sys(devp, &devp->demod_status, &sys);
...
static int gxtv_demod_dvbc_set_frontend(struct dvb_frontend *fe)
{
	...
	PR_INFO("%s: delsys:%d, freq:%d, symbol_rate:%d, bw:%d, modul:%d.\n",
	...
	if (!cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
		if (param.mode == 3 && devp->demod_status.tmp != ADC_MODE)
			ret = Gxtv_Demod_Dvbc_Init(ADC_MODE);
	}

	if (devp->autoflags == 1 && devp->auto_flags_trig == 0 &&
		devp->freq_dvbc == param.ch_freq) {
	...
	devp->demod_status.ci_mode = devp->ci_mode;
	dvbc_set_ch(&devp->demod_status, &param, fe);
...
static int dvbt_isdbt_read_status(struct dvb_frontend *fe, enum fe_status *status, bool re_tune)
{
	...
	PR_ISDBT("fsm=0x%x, strength=%ddBm snr=%d.%ddB, time_passed=%dms\n",
		fsm, strength, snr10 / 10, snr10 % 10, devp->time_passed);

	s = dvbt_isdbt_rd_reg(0x0) >> 12 & 1;
	if (s == 1)
		lock = 1;
	else if (devp->time_passed < ISDBT_TIME_CHECK_SIGNAL ||
		(devp->time_passed < TIMEOUT_ISDBT && has_signal && devp->last_lock == 0))
		lock = 0;
	else
		lock = -1;
	...
	if (lock < 0) {
		...
		} else if (devp->last_lock <= -lost_continuous_cnt) {
			devp->last_lock = -lost_continuous_cnt;
		...
		if (devp->last_lock <= -lost_continuous_cnt)
			*status = FE_TIMEDOUT;
		else
			*status = 0;
	} else if (lock > 0) {
		...
		} else if (devp->last_lock >= lock_continuous_cnt) {
			devp->last_lock = lock_continuous_cnt;
		...
		if (devp->last_lock >= lock_continuous_cnt)
	...
	if (s == 1) {
		...
		*status =
		...
		devp->real_para.snr =
			(((dvbt_t2_rdb(CHC_CIR_SNR1) & 0x7) << 8)
			| dvbt_t2_rdb(CHC_CIR_SNR0)) * 30 / 64;
		devp->real_para.modulation = dvbt_t2_rdb(0x2912) & 0x3;
		devp->real_para.coderate = dvbt_t2_rdb(0x2913) & 0x7;
		devp->real_para.tps_cell_id =
...
static int dvbt2_read_status(struct dvb_frontend *fe, enum fe_status *status)
{
	...
	static int no_signal_cnt, unlock_cnt, retry_PEAK, retry_ldpc;
	...
	PR_DVBT("s=%d, p1=%d, devp->p1=%d, devp->last_lock=%d, val=0x%08x\n",
		s, p1_peak, devp->p1_peak, devp->last_lock, val);
	...
	if (s == 1) {
		if (devp->last_lock >= 0 &&
			devp->last_lock < CONTINUE_TIMES_LOCK &&
			devp->time_passed < TIMEOUT_DVBT2) {
	...
	} else if (devp->last_lock == 0) {
		if (devp->p1_peak == 0 && devp->time_passed < TIMEOUT_SIGNAL_T2) {
			if (p1_peak == 1)
				devp->p1_peak = 1;
		} else if (devp->p1_peak == 1 && devp->time_passed < TIMEOUT_DVBT2) {
			if (p1_peak == 0) {
				retry_PEAK++;
				PR_DVBT("!! >> retry PEAK %d << !!\n", retry_PEAK);
				if (retry_PEAK >= 2) {
					dvbt2_reset(devp, fe);
					retry_PEAK = 0;
				}
			...
			*status = FE_TIMEDOUT;
			devp->last_lock = -CONTINUE_TIMES_UNLOCK;
		}
	} else if (devp->last_lock > 0) {
		if (devp->time_passed < TIMEOUT_DVBT2) {
			PR_DVBT("!!>> lost reset <<!!\n");
			devp->last_lock = 0;
		} else {
			PR_DVBT("!!>> maybe lost <<!!\n");
		...
		if (devp->last_lock > -CONTINUE_TIMES_UNLOCK) {
			PR_DVBT("!!>> lost +1 <<!!\n");
			devp->last_lock -= 1;
		}
	}

	if (devp->last_lock == CONTINUE_TIMES_LOCK) {
		*status = FE_HAS_LOCK | FE_HAS_SIGNAL | FE_HAS_CARRIER |
				FE_HAS_VITERBI | FE_HAS_SYNC;
		devp->real_para.snr = snr / 10;
		devp->real_para.modulation = modu;
	...
	if (*status == 0)
		PR_INFO("!! >> WAITT2 << !!\n");
	else if (devp->last_status != *status)
		PR_INFO("!! >> %sT2 << !!, freq:%d\n", *status == FE_TIMEDOUT ? "UNLOCK" : "LOCK",
			fe->dtv_property_cache.frequency);
...
static int dvbt_isdbt_set_frontend(struct dvb_frontend *fe)
{
	...
	PR_INFO("%s: delsys:%d, freq:%d, symbol_rate:%d, bw:%d, modul:%d, invert:%d.\n",
			__func__, c->delivery_system, c->frequency, c->symbol_rate,
	...
	tuner_set_params(fe);
	msleep(20);
	dvbt_isdbt_set_ch(&devp->demod_status, &param);
	return 0;
}
...
static int dvbt_set_frontend(struct dvb_frontend *fe)
{
	...
	dvbt_set_ch(&devp->demod_status, &param, fe);
...
int dvbt_isdbt_Init(void)
{
	...
	devp->demod_status.delsys = SYS_ISDBT;
	sys.adc_clk = ADC_CLK_24M;
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_T5D))
		...
		dd_tvafe_hiu_reg_write(D_HHI_DEMOD_CLK_CNTL, 0x501);
...
static unsigned int dvbt_init(void)
{
	...
	devp->demod_status.delsys = SYS_DVBT;
	...
	dd_tvafe_hiu_reg_write(D_HHI_DEMOD_CLK_CNTL_1, 0x704);
	dd_tvafe_hiu_reg_write(D_HHI_DEMOD_CLK_CNTL, 0x501);
	demod_set_sys(devp, &devp->demod_status, &sys);
	devp->last_status = 0;
...
static unsigned int dtvdemod_dvbt2_init(void)
{
	...
	devp->demod_status.delsys = SYS_DVBT2;
	sys.adc_clk = ADC_CLK_54M;
	sys.demod_clk = DEMOD_CLK_216M;
	devp->demod_status.ch_if = DEMOD_5M_IF;
	devp->demod_status.adc_freq = sys.adc_clk;
		...
		enum fe_status *status)
{
	...
	if ((c->modulation <= QAM_AUTO) && (c->modulation != QPSK)) {
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			...
			if (s == 0 && devp->last_lock == 1 && (atsc_read_reg(0x0980) >= 0x76)) {
				s = 1;
	...
	if (aml_demod_debug & DBG_ATSC) {
		if (devp->atsc_dbg_lst_status != s || devp->last_lock != ilock) {
			/* check tuner */
			strength = tuner_get_ch_power(fe);
...
static int gxtv_demod_atsc_set_frontend(struct dvb_frontend *fe)
{
	...
	if ((c->modulation <= QAM_AUTO) && (c->modulation != QPSK)) {
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			...
			devp->demod_status.clk_freq = DEMOD_CLK_167M;
			nco_rate = (devp->demod_status.adc_freq * 256) /
					devp->demod_status.clk_freq + 2;
			PR_ATSC("demod_clk:%d, ADC_CLK:%d, nco_rate:%d\n",
				devp->demod_status.clk_freq,
				devp->demod_status.adc_freq, nco_rate);
		...
		dvbc_set_ch(&devp->demod_status, &param_j83b, fe);
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
			...
			if (devp->demod_status.adc_freq == ADC_CLK_24M) {
				if (tuner_find_by_name(fe, "r842") &&
					tuner_freq[1] == DEMOD_4_57M_IF * 1000) {
					devp->demod_status.ch_if = DEMOD_4_57M_IF;
					//4.57M IF, 2^23 * IF / Fs.
			...
			atsc_set_ch(&devp->demod_status, &param_atsc);
...
static int atsc_j83b_set_frontend_mode(struct dvb_frontend *fe, int mode)
{
	...
	PR_INFO("%s : freq:%d, mode:%d\n", __func__, c->frequency, mode);
...
static void atsc_j83b_switch_qam(struct dvb_frontend *fe, enum qam_md_e qam)
{
	...
	if (qam == QAM_MODE_64) {
		qam_write_bits(0xd, 5057, 0, 16);
		qam_write_bits(0x11, 5057, 8, 16);
	} else {
		qam_write_bits(0xd, 5361, 0, 16);
		qam_write_bits(0x11, 5361, 8, 16);
	}
	demod_dvbc_set_qam(qam);
...
static int atsc_j83b_read_status(struct dvb_frontend *fe, enum fe_status *status, bool re_tune)
{
	...
	devp->time_passed = curTime - devp->time_start;
	s = qam_read_reg(0x31) & 0xf;

	if (s != 5)
		real_para_clear(&devp->real_para);

	devp->real_para.snr = dvbc_get_snr();
	PR_ATSC("fsm=%d, snr=%d.%d, devp->time_passed=%u\n", s,
		devp->real_para.snr / 100, devp->real_para.snr % 100, devp->time_passed);
	...
	if (s == 5) {
		...
		*status = FE_HAS_LOCK | FE_HAS_SIGNAL | FE_HAS_CARRIER |
			FE_HAS_VITERBI | FE_HAS_SYNC;
		devp->real_para.modulation = amdemod_qam_fe(qam);
		...
		if (time_passed_qam < 125) {
			*status = 0;
			...
			if (devp->last_lock == 0 && check_first > 0) {
				*status = 0;
				check_first--;
			} else {
				*status = FE_TIMEDOUT;
		...
		*status = 0;
		...
		if (devp->real_para.snr > J83B_CHECK_SNR_THRESHOLD) {
			is_signal = true;
			timeout = TIMEOUT_ATSC;
		}

		if ((devp->last_lock == 0 || is_signal) && devp->time_passed < timeout) {
			*status = 0;
		} else if (devp->last_status == 0x1F) {
			*status = 0;
			PR_ATSC("retry fsm=0x%x, snr=%d\n", qam_read_reg(0x31), dvbc_get_snr());
		} else {
			*status = FE_TIMEDOUT;
		}

		if (devp->time_passed > TIMEOUT_ATSC_STD &&
			(devp->time_passed % TIMEOUT_ATSC_STD) < 100) {
...
static void atsc_read_status(struct dvb_frontend *fe, enum fe_status *status, unsigned int re_tune)
{
	...
	if (sys_sts >= ATSC_SYNC_LOCK) {
		...
		peak = 1;//atsc signal
		if (devp->time_passed >= TIMEOUT_ATSC)
			fsm_status = -1;
		else
			fsm_status = 0;
		...
		if (devp->time_passed <= atsc_check_signal_time) {
			fsm_status = 0;
		} else {
			fsm_status = -1;
	...
	*delay = HZ / 20;
	...
	if (re_tune) {
		...
		if (c->modulation ==  QPSK) {
			...
			PR_ATSC("detect modulation is j83 first\n");
			atsc_j83b_read_status(fe, status, re_tune);
		} else if (c->modulation > QAM_AUTO) {
			PR_ATSC("modulation is 8VSB.\n");
			atsc_read_status(fe, status, re_tune);
		} else {
			PR_ATSC("modulation is %d unsupported!\n",
					c->modulation);
		}
...
static void dvbt_rst_demod(struct amldtvdemod_device_s *devp, struct dvb_frontend *fe)
{
...
static int dtvdemod_atsc_init(struct amldtvdemod_device_s *devp)
{
	...
	memset(&sys, 0, sizeof(sys));
	memset(&devp->demod_status, 0, sizeof(devp->demod_status));
	devp->demod_status.delsys = SYS_ATSC;
	sys.adc_clk = ADC_CLK_24M;
...
{
	struct amldtvdemod_device_s *devp = (struct amldtvdemod_device_s *)fe->demodulator_priv;
	struct poll_machie_s *pollm = &devp->poll_machie;
...
static int gxtv_demod_dtmb_set_frontend(struct dvb_frontend *fe)
{
	...
	if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1)) {
		if (fe->ops.tuner_ops.get_if_frequency)
			fe->ops.tuner_ops.get_if_frequency(fe, tuner_freq);
		if (tuner_freq[0] == 0)
			devp->demod_status.spectrum = 0;
		else if (tuner_freq[0] == 1)
			devp->demod_status.spectrum = 1;
		else
			pr_err("wrong specturm val get from tuner\n");
	}

	dtmb_set_ch(&devp->demod_status, &param);
...
int Gxtv_Demod_Dtmb_Init(void)
{
	...
	memset(&sys, 0, sizeof(sys));
	memset(&devp->demod_status, 0, sizeof(devp->demod_status));
	devp->demod_status.delsys = SYS_DTMB;
	...
	devp->demod_status.ch_if = DEMOD_5M_IF;
	devp->demod_status.tmp = ADC_MODE;
	devp->demod_status.spectrum = devp->spectrum;
	devp->demod_status.adc_freq = sys.adc_clk;
	devp->demod_status.clk_freq = sys.demod_clk;
...
unsigned int dvbc_auto_fast(struct dvb_frontend *fe, unsigned int *delay, bool re_tune)
{
	...
	if (re_tune) {
		...
		devp->auto_sr_done = 0;
		devp->sr_val_hw_stable = 0;
		devp->sr_val_hw_count = 0;
		devp->sr_val_uf_count = 0;
		sym_speed_high = 0;
		demod_dvbc_set_qam(qam_mode);

		demod_dvbc_fsm_reset();

		if (!devp->auto_sr && qam_mode == QAM_MODE_256)
			*delay = HZ / 2;//500ms
		time_start = jiffies_to_msecs(jiffies);
		PR_DVBC("%s: retune reset, sr %d.\n", __func__, devp->sr_val_hw);
		return 2;
	}
	...
	/* symbol rate, 0=auto */
	if (devp->auto_sr) {
		if (!devp->auto_sr_done) {
			devp->sr_val_hw = dvbc_get_symb_rate();
			eq_state = qam_read_reg(0x5d) & 0xf;
			fsm_state = qam_read_reg(0x31) & 0xf;

			if (abs(devp->sr_val_hw - devp->sr_val_hw_stable) <= 100)
				devp->sr_val_hw_count++;
			else
				devp->sr_val_hw_count = 0;

			PR_DVBC("%s: get sr %d, count %d, eq_state(0x5d) 0x%x, fsm_state(0x31) 0x%x.\n",
					__func__, devp->sr_val_hw, devp->sr_val_hw_count,
					eq_state, fsm_state);

			devp->sr_val_hw_stable = devp->sr_val_hw;

			if (eq_state >= 2 || devp->sr_val_hw_count >= 3) {
				// slow down auto sr speed and range.
				dvbc_cfg_sr_scan_speed(SYM_SPEED_NORMAL);
				dvbc_cfg_sr_cnt(SYM_SPEED_NORMAL);
				dvbc_cfg_sw_hw_sr_max(devp->sr_val_hw_stable);
				dvbc_cfg_tim_sweep_range(SYM_SPEED_NORMAL);

				devp->auto_sr_done = 1;
				PR_DVBC("%s: auto_sr_done[%d], sr_val_hw_stable %d, cost %d ms.\n",
						__func__, fe->dtv_property_cache.frequency,
						devp->sr_val_hw_stable,
				...
				if (devp->sr_val_hw < 6820 &&
					sym_speed_high == 0) {
					// fast down auto sr speed and range.
					dvbc_cfg_sr_scan_speed(SYM_SPEED_HIGH);
					dvbc_cfg_sr_cnt(SYM_SPEED_HIGH);
					dvbc_cfg_tim_sweep_range(SYM_SPEED_HIGH);
					sym_speed_high = 1;
			...
			/* sr underflow */
			if (devp->sr_val_hw_stable < 3400) {
				if (++devp->sr_val_uf_count > 1) {
					PR_DVBC("%s: auto sr underflow(count %d) unlocked.\n",
							__func__, devp->sr_val_uf_count);
		...
		if ((jiffies_to_msecs(jiffies) - time_start < 1250) && !devp->auto_sr_done) {
			// try every 100ms.
			*delay = HZ / 10;
	...
	fsm_state = qam_read_reg(0x31);
	PR_DVBC("%s: fsm_state(0x31): 0x%x, times: %d.\n",
			__func__, fsm_state, times);
	...
	if ((fsm_state & 0xf) < 3) {
		...
		times = 0;
		*delay = HZ / 4;
		devp->real_para.modulation = amdemod_qam_fe(qam_mode);
		devp->real_para.symbol = devp->auto_sr ?
			devp->sr_val_hw_stable * 1000 :
	...
	if (devp->auto_sr && devp->auto_sr_done) {
		devp->sr_val_hw = dvbc_get_symb_rate();
		if (abs(devp->sr_val_hw - devp->sr_val_hw_stable) >= 100) {
			PR_DVBC("%s: switch qam rewrite hw sr from %d to %d.\n",
					__func__, devp->sr_val_hw, devp->sr_val_hw_stable);
			dvbc_cfg_sw_hw_sr_max(devp->sr_val_hw_stable);
		}
	}
...
static unsigned int dvbc_fast_search(struct dvb_frontend *fe, unsigned int *delay, bool re_tune)
{
		...
		if (!devp->auto_sr_done) {
			...
			if (eq_state >= 2 || devp->sr_val_hw_count >= 3) {
				...
				PR_DVBC("%s: auto_sr_done, sr_val_hw_stable %d, cost %d ms.\n",
						__func__, devp->sr_val_hw_stable,
						jiffies_to_msecs(jiffies) - time_start);
			} else {
				if (devp->sr_val_hw < 6820 &&
					sym_speed_high == 0) {
	...
	if ((fsm_state & 0xf) < 3) {
		...
		times = 0;
		*delay = HZ / 4;
		devp->real_para.modulation = fe->dtv_property_cache.modulation;
		devp->real_para.symbol = devp->auto_sr ?
				devp->sr_val_hw_stable * 1000 :
				fe->dtv_property_cache.symbol_rate;
		return 1;
	...
	*delay = HZ / 4;
	...
	if (re_tune) {
		...
		if (demod_dvbc_speedup_en == 1) {
			...
			*status = 0;
			*delay = HZ / 8;
			qam_write_reg(0x65, 0x400c); // offset
			// agc gain
			qam_write_reg(0x24, (qam_read_reg(0x24) | (1 << 17)));
	...
	if (*status & FE_HAS_LOCK) {
		timer_disable(devp, D_TIMER_SET);
	} else {
		if (!timer_is_en(devp, D_TIMER_SET))
			timer_begain(devp, D_TIMER_SET);
	}

	if (timer_is_enough(devp, D_TIMER_SET)) {
		gxtv_demod_dvbc_set_frontend(fe);
		timer_disable(devp, D_TIMER_SET);
	}
...
static void dvbc_set_speedup(void)
{
	qam_write_reg(0x65, 0x400c);
	qam_write_reg(0x60, 0x10466000);
	qam_write_reg(0xac, (qam_read_reg(0xac) & (~0xff00)) | 0x800);
	qam_write_reg(0xae, (qam_read_reg(0xae) & (~0xff000000)) | 0x8000000);
}

static void dvbc_set_srspeed(int high)
{
	if (high) {
		qam_write_reg(SYMB_CNT_CFG, 0xffff03ff);
		qam_write_reg(SR_SCAN_SPEED, 0x234cf523);
	} else {
...
static int dvbc_read_status(struct dvb_frontend *fe, enum fe_status *status, bool re_tune)
{
	...
	unsigned int curTime, time_passed_qam;
	static int peak;
	static enum qam_md_e AQAM_mode = QAM_MODE_256;
	...
	if (re_tune) {
		...
		time_start_qam = 0;
		if (c->modulation == QAM_AUTO)
			AQAM_mode = QAM_MODE_256;
		else
			AQAM_mode = amdemod_qam(c->modulation);
		if (c->symbol_rate == 0)
			ASR_end = 3480;
		else
	...
	if (tuner_find_by_name(fe, "r842")) {
		...
		if (str <= -80)
			str = dvbc_get_power_strength(qam_read_reg(0x27) & 0x7ff, str);
	}
	...
	if (s == 5) {
		...
		*status = FE_HAS_LOCK | FE_HAS_SIGNAL | FE_HAS_CARRIER |
			FE_HAS_VITERBI | FE_HAS_SYNC;
		devp->real_para.modulation = dvbc_get_dvbc_qam(AQAM_mode);
		devp->real_para.symbol = sr * 1000;

		PR_DVBC("locked at sr:%d,Qam:%s\n", sr, get_qam_name(AQAM_mode));
	} else if (s < 3) {
		time_start_qam = 0;
		if (devp->last_lock == 0 && (devp->time_passed < 1300 ||
		...
		if (c->symbol_rate == 0 && sr < ASR_end) {
			devp->sr_val_hw = 7000;
			qam_write_bits(0xd, 7000, 0, 16);
			qam_write_bits(0x11, 7000, 8, 16);
			PR_DVBC("sr reset from %d.freq=%d\n", 7000, c->frequency);
			ASR_end = ASR_end == 3480 ? 6820 : 3480;
			dvbc_set_srspeed(ASR_end == 3480 ? 1 : 0);
			demod_dvbc_fsm_reset();
		} else if (c->symbol_rate != 0 && sr < ASR_end) {
			devp->sr_val_hw = c->symbol_rate / 1000;
			qam_write_bits(0xd, c->symbol_rate / 1000, 0, 16);
			qam_write_bits(0x11, c->symbol_rate / 1000, 8, 16);
			dvbc_set_srspeed(0);
			demod_dvbc_fsm_reset();
		}
	} else if (s == 4 || s == 7) {
		*status = 0;
		...
		PR_DVBC("true sr:%d, try Qam:%s, time_passed:%u\n",
			sr, get_qam_name(AQAM_mode), devp->time_passed);
		PR_DVBC("time_start_qam=%u\n", time_start_qam);
		if (devp->last_lock == 0 && (devp->time_passed < TIMEOUT_DVBC ||
		...
		if (c->modulation != QAM_AUTO) {
			...
			time_passed_qam = curTime - time_start_qam;
			if ((AQAM_mode == QAM_MODE_256 && time_passed_qam >
				(650 + 300 * (AQAM_times % 3))) ||
				(AQAM_mode != QAM_MODE_256 && time_passed_qam >
				(350 + 100 * (AQAM_times % 3)))) {
				time_start_qam = curTime;
				AQAM_mode = dvbc_switch_qam(AQAM_mode);
				PR_DVBC("to next qam:%s\n", get_qam_name(AQAM_mode));
				demod_dvbc_set_qam(AQAM_mode);
				if (AQAM_mode == QAM_MODE_16) {
					time_start_qam = 0;
					PR_DVBC("set QAM_16, so reset\n");
					if (sr >= 3500) {
						PR_DVBC("set sr start from:%d\n", sr);
						qam_write_bits(0xd, sr, 0, 16);
						qam_write_bits(0x11, sr, 8, 16);
					}
					demod_dvbc_fsm_reset();
				} else if (AQAM_mode == QAM_MODE_256) {
					AQAM_times++;
				}
			}
		}
...
int dtvdemod_dvbs_set_frontend(struct dvb_frontend *fe)
{
	...
	if (devp->blind_same_frec == 0)
		PR_INFO("%s: delsys: %d, freq: %d, symbol_rate: %d, bw: %d.\n",
				__func__, c->delivery_system, c->frequency, c->symbol_rate,
				c->bandwidth_hz);

	devp->demod_status.symb_rate = c->symbol_rate / 1000;
	devp->demod_status.is_blind_scan = 0; /* The actual frequency lock normal mode */
	devp->last_lock = -1;
	...
	dtvdemod_dvbs_set_ch(&devp->demod_status);
	devp->time_start = jiffies_to_msecs(jiffies);
		...
		unsigned int freq_khz, int *next_step_khz, int *signal_state)
{
	...
	if (!devp->blind_scan_stop && devp->demod_status.is_blind_scan &&
			devp->demod_status.is_singlecable) {
		PR_DVBS("%s: IF_freq %d\n", __func__, c->frequency);
...
//		*status = FE_TIMEDOUT;
//		devp->last_lock = -1;
//		PR_DVBS("%s: tuner strength [%d] no signal(%d).\n",
//			__func__, strength, THRD_TUNER_STRENGTH_DVBS);
//		return 0;
//	}
	...
	devp->real_para.ber = (pkt_err_cnt - pkt_err_cnt_last) * 10 * (400000000 / (br / 10));
		...
		*status =
		...
		if (devp->blind_scan_stop) {
			...
			if ((offset > 6500 && c->symbol_rate < SR_LOW_THRD) ||
				...
				ilock = 0;
				*status = FE_TIMEDOUT;
				PR_INFO("%s: offset %d KHz too large, sr %d.\n",
						__func__, offset, c->symbol_rate);
			}
		}
/*
		// S/S2 ber config.
...
static int dtvdemod_dvbs2_init(void)
{
	...
	devp->demod_status.delsys = SYS_DVBS2;
	...
	dd_tvafe_hiu_reg_write(D_HHI_DEMOD_CLK_CNTL_1, 0x702);
	dd_tvafe_hiu_reg_write(D_HHI_DEMOD_CLK_CNTL, 0x501);
...
static void set_agc_pinmux(enum fe_delivery_system delsys, unsigned int on)
{
	...
	if (on) {
		if (agc_name && IS_ERR_OR_NULL(devp->pin_agc)) {
		...
		if (diseqc_out_name && IS_ERR_OR_NULL(devp->pin_diseqc_out)) {
			pin = devm_pinctrl_get_select(devp->dev, diseqc_out_name);
			if (IS_ERR_OR_NULL(pin))
				PR_ERR("get agc pins fail: %s\n", diseqc_out_name);
			else
				devp->pin_diseqc_out = pin;
		}

		if (diseqc_in_name && IS_ERR_OR_NULL(devp->pin_diseqc_in)) {
			pin = devm_pinctrl_get_select(devp->dev, diseqc_in_name);
			if (IS_ERR_OR_NULL(pin))
				PR_ERR("get pins fail: %s\n", diseqc_in_name);
			else
				devp->pin_diseqc_in = pin;
		}
	} else {
		if (!IS_ERR_OR_NULL(devp->pin_agc)) {
			devm_pinctrl_put(devp->pin_agc);
			devp->pin_agc = NULL;
		}

		if (!IS_ERR_OR_NULL(diseqc_out_name)) {
			if (!IS_ERR_OR_NULL(devp->pin_diseqc_out)) {
				devm_pinctrl_put(devp->pin_diseqc_out);
				devp->pin_diseqc_out = NULL;
			}
		}

		if (!IS_ERR_OR_NULL(diseqc_in_name)) {
			if (!IS_ERR_OR_NULL(devp->pin_diseqc_in)) {
				devm_pinctrl_put(devp->pin_diseqc_in);
				devp->pin_diseqc_in = NULL;
			}
		}
	}
...
static bool enter_mode(enum fe_delivery_system delsys)
{
	...
	case SYS_DVBC_ANNEX_C:
		...
		timer_set_max(devp, D_TIMER_DETECT, devp->timeout_dvbc_ms);
		/* reset is 4s */
		timer_set_max(devp, D_TIMER_SET, 4000);
	...
	case SYS_DVBC_ANNEX_B:
		...
		if (cpu_after_eq(MESON_CPU_MAJOR_ID_TL1))
			timer_set_max(devp, D_TIMER_DETECT, devp->timeout_atsc_ms);
	...
	case SYS_DVBT:
		ret = dvbt_init();
		timer_set_max(devp, D_TIMER_DETECT, devp->timeout_dvbt_ms);
	...
	case SYS_ISDBT:
		...
		timer_set_max(devp, D_TIMER_DETECT, 2000);
		/*reset is 4s*/
		timer_set_max(devp, D_TIMER_SET, 4000);
	...
	case SYS_DVBS2:
		dtvdemod_dvbs2_init();
		dvbs2_diseqc_init();
		timer_set_max(devp, D_TIMER_DETECT, devp->timeout_dvbs_ms);
...
static int leave_mode(enum fe_delivery_system delsys)
{
	...
	devp->en_detect = 0;
	devp->last_delsys = SYS_UNDEFINED;
...
static void delsys_exit(unsigned int ldelsys, unsigned int cdelsys)
{
	struct amldtvdemod_device_s *devp = dtvdemod_get_dev();
	struct dvb_frontend *fe = &devp->frontend;

	if (demod_is_t5d_cpu(devp) && ldelsys == SYS_DVBT2) {
		demod_top_write_reg(DEMOD_TOP_CFG_REG_4, 0x182);
		dvbt_t2_wr_byte_bits(0x09, 1, 4, 1);
		demod_top_write_reg(DEMOD_TOP_CFG_REG_4, 0x97);
		riscv_ctl_write_reg(0x30, 4);
		...
		dvbt_t2_wr_byte_bits(0x3613, 0, 4, 3);
		dvbt_t2_wr_byte_bits(0x3617, 0, 0, 3);
	} else if (demod_is_t5d_cpu(devp) && ldelsys == SYS_ISDBT) {
		dvbt_isdbt_wr_reg((0x2 << 2), 0x111021b);
		dvbt_isdbt_wr_reg((0x2 << 2), 0x011021b);
		dvbt_isdbt_wr_reg((0x2 << 2), 0x011001b);
	}
...
static void dtvdemod_clktree_probe(struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(devp->vdac_clk_gate))
		PR_INFO("%s: clk vdac_clk_gate error.\n", __func__);
}
...
static void dvbs_blind_scan_new_work(struct work_struct *work)
{
	...
	devp->blind_result_frequency = 0;
	devp->blind_result_symbol_rate = 0;

	c->symbol_rate = srate;
	devp->demod_status.symb_rate = srate / 1000;
	devp->demod_status.is_blind_scan = !devp->blind_scan_stop;

	if (devp->singlecable_param.version) {
		devp->demod_status.is_singlecable = 1;
...
static int aml_dtvdemod_probe(struct platform_device *pdev)
{
	...
	devp->timeout_ddr_leave = TIMEOUT_DDR_LEAVE;
	devp->demod_thread = 1;
	devp->blind_scan_stop = 1;
	devp->fast_search_finish = 1;
...
static void aml_dtvdemod_shutdown(struct platform_device *pdev)
{
	...
	if (devp->state != DTVDEMOD_ST_IDLE) {
		if (devp->last_delsys != SYS_UNDEFINED)
			delsys_exit(devp->last_delsys, SYS_UNDEFINED);
	}
...
static int dtvdemod_leave_mode(void)
{
	...
	if (delsys != SYS_UNDEFINED) {
		...
		delsys_exit(delsys, SYS_UNDEFINED);
	}
...
static int delsys_set(struct dvb_frontend *fe, unsigned int delsys)
{
	...
	case SYS_ANALOG:
		if (get_dtvpll_init_flag()) {
			...
			delsys_exit(ldelsys, SYS_UNDEFINED);
		}
	...
	if (cdelsys != SYS_UNDEFINED) {
		if (ldelsys != SYS_UNDEFINED)
			delsys_exit(ldelsys, cdelsys);

		if (enter_mode(cdelsys)) {
			PR_INFO("enter_mode failed,leave!\n");
			delsys_exit(cdelsys, SYS_UNDEFINED);
...
static int is_not_active(struct dvb_frontend *fe)
{
	...
	if (!get_dtvpll_init_flag()) {
		PR_INFO("%s: dtvpll uninit.\n", __func__);

		return 1;
	}

	if (ldelsys == SYS_UNDEFINED) {
		PR_INFO("%s: ldelsys == SYS_UNDEFINED.\n", __func__);

		return 2;
	}

	if (ldelsys != cdelsys) {
		PR_INFO("%s: ldelsys(%d) != cdelsys(%d).\n",
			__func__, ldelsys, cdelsys);
	...
	if (!devp->demod_thread) {
		PR_INFO("%s: devp->demod_thread(%d).\n",
			__func__, devp->demod_thread);
				 ...
				 struct dtv_frontend_properties *p)
{
	...
	case SYS_DVBS2:
		if (!devp->blind_scan_stop) {
			p->frequency = devp->blind_result_frequency;
			p->symbol_rate = devp->blind_result_symbol_rate;
		...
		PR_DVBS("%s delsys %d,freq %d,srate %d\n",
				__func__, delsys,
		...
		enum fe_status *status)
{
	...
	PR_DBG("%s: delsys %d, status 0x%x, ret %d.\n",
			__func__, delsys, *status, ret);
		...
		u16 *strength)
{
	...
	case SYS_DVBT2:
		ret = dtvdemod_dvbt2_read_signal_strength(fe, (s16 *)strength);
		break;
			...
			struct dtv_property *tvp)
{
	...
	case DTV_DELIVERY_SYSTEM:
		...
		if (devp->last_delsys == SYS_DVBS || devp->last_delsys == SYS_DVBS2) {
			...
			PR_DVBS("get delsys:%d,modulation:%d.\n",
					tvp->u.data, tvp->reserved[0]);
		} else if (devp->last_delsys == SYS_DVBT2 &&
			devp->last_status == 0x1F) {
			modulation = devp->real_para.modulation;
			cr = devp->real_para.coderate;
			...
			tvp->reserved[2] = devp->real_para.fef_info;

			PR_DVBT("get delsys:%d,modulation:%d,code_rate:%d,fef_info:%d.\n",
				tvp->u.data, tvp->reserved[0],
				tvp->reserved[1], tvp->reserved[2]);

		} else if (devp->last_delsys == SYS_DVBT &&
			devp->last_status == 0x1F) {
			modulation = devp->real_para.modulation;
			cr = devp->real_para.coderate;
			...
			tvp->reserved[2] = devp->real_para.tps_cell_id;

			PR_DVBT("get delsys:%d,modulation:%d,code_rate:%d,cell_id:%d.\n",
					tvp->u.data,
					tvp->reserved[0], tvp->reserved[1], tvp->reserved[2]);
		} else if ((devp->last_delsys == SYS_DVBC_ANNEX_A ||
			devp->last_delsys == SYS_DVBC_ANNEX_C) &&
			devp->last_status == 0x1F) {
			tvp->reserved[0] = devp->real_para.modulation;
			tvp->reserved[1] = devp->real_para.symbol;

			PR_DVBC("get delsys:%d,modulation:%d,symbol:%d.\n",
				tvp->u.data, tvp->reserved[0], tvp->reserved[1]);
		}
		break;

	case DTV_DVBT2_PLP_ID:
		/* plp nums & ids */
		tvp->u.buffer.reserved1[0] = devp->real_para.plp_num;
		if (tvp->u.buffer.reserved2 && devp->real_para.plp_num > 0) {
			unsigned char i, *plp_ids;

			plp_ids = kmalloc(devp->real_para.plp_num, GFP_KERNEL);
			if (plp_ids) {
				for (i = 0; i < devp->real_para.plp_num; i++)
					plp_ids[i] = i;
				if (copy_to_user(tvp->u.buffer.reserved2,
					plp_ids, devp->real_para.plp_num))
					PR_ERR("copy plp ids to user err\n");

				kfree(plp_ids);
			}
		}
		PR_INFO("get plp num = %d\n", devp->real_para.plp_num);
		break;
	...
	case DTV_ISDBT_PARTIAL_RECEPTION:
		if (devp->last_delsys == SYS_ISDBT && devp->last_status == 0x1F) {
...
struct dvb_frontend *aml_dtvdm_attach(const struct demod_config *config)
{
	...
	fe = &devp->frontend;
	devp->frontend.demodulator_priv = devp;
	devp->last_delsys = SYS_UNDEFINED;
