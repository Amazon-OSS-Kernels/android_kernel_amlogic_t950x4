...
static void info_show(void)
{
	...
	c = &devp->frontend.dtv_property_cache;
	...
	strength = tuner_get_ch_power(&devp->frontend);
	...
	if (!aml_dtvdm_read_ber(&devp->frontend, &ber))
		PR_INFO("ber=%d\n", ber);
}
		...
		struct class_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strcmp(parm[0], "symb_rate")) {
		...
		if (fe->ops.init && devp->last_delsys == SYS_UNDEFINED)
			fe->ops.init(fe);
		...
		if (devp->last_delsys == SYS_DVBC_ANNEX_A) {
			if (parm[1] && (kstrtouint(parm[1], 16, &val)) == 0) {
				if (val == 0) {
