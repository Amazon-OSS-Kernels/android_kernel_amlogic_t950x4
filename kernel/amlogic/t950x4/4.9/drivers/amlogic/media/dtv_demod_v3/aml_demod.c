			    ...
			    unsigned int cmd, unsigned long arg)
{
		...
		if (copy_from_user(&tuner_para, (void __user *)arg,
			...
			if (tuner_para.mode <= FE_ISDBT) {
				PR_INFO("set tuner md = %d\n",
					tuner_para.mode);
				devp->frontend.ops.info.type = tuner_para.mode;
			} else {
				PR_ERR("wrong md: %d\n", tuner_para.mode);
			}

			devp->frontend.dtv_property_cache.frequency =
				tuner_para.ch_freq;
			devp->frontend.ops.tuner_ops.set_config(&devp->frontend, NULL);
			tuner_set_params(&devp->frontend);
		}
		break;
	...
	case AML_DEMOD_DVBT_SET_CH:
		...
		dvbt_isdbt_set_ch(&demod_sta, (struct aml_demod_dvbt *)arg);
		break;
	...
	case AML_DEMOD_DTMB_SET_CH:
		dtmb_set_ch(&demod_sta, (struct aml_demod_dtmb *)arg);
		break;

	case AML_DEMOD_ATSC_SET_CH:
		atsc_set_ch(&demod_sta, (struct aml_demod_atsc *)arg);
		break;
