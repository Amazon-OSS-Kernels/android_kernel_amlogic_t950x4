	...
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct snd_soc_codec *codec = snd_soc_component_to_codec(component);
	struct tl1_acodec_priv *aml_acodec = snd_soc_codec_get_drvdata(codec);
	u32 val = (audiobus_read(EE_AUDIO_TOACODEC_CTRL0) >> 16) & (0x7);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	/* there are only tdma/tdmb fot T5 */
	audiobus_update_bits(EE_AUDIO_TOACODEC_CTRL0, (0x7 << 16), (val << 16));
...
static const struct soc_enum DAC2_Gain_enum = SOC_ENUM_SINGLE(
			...
			DAC2_Gain_texts);
static const struct soc_enum DAC_source_sel_enum = SOC_ENUM_SINGLE(
