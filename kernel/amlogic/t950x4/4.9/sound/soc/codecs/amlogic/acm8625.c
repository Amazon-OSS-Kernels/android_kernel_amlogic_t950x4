/*
 * Driver for the ACM8625 Audio Amplifier
 *
...
static void acm8625_set_volume(struct snd_soc_codec *codec, int vol)
{
	unsigned int index;
	uint32_t volume_hex;
	uint8_t byte4;
	...
	//w 58 00 04
	snd_soc_write(codec, ACM8625_REG_00, ACM8625_PAGE_04);
	//w 58 7C xx xx xx xx
	snd_soc_write(codec, ACM8625_REG_7C, byte4);
	snd_soc_write(codec, ACM8625_REG_7D, byte3);
	snd_soc_write(codec, ACM8625_REG_7E, byte2);
	snd_soc_write(codec, ACM8625_REG_7F, byte1);
	//w 58 80 xx xx xx xx
	snd_soc_write(codec, ACM8625_REG_80, byte4);
	snd_soc_write(codec, ACM8625_REG_81, byte3);
	snd_soc_write(codec, ACM8625_REG_82, byte2);
	snd_soc_write(codec, ACM8625_REG_83, byte1);
	snd_soc_write(codec, ACM8625_REG_00, ACM8625_PAGE_00);
...
static int acm8625_mute(struct snd_soc_codec *codec, int mute)
{
	u8 reg04_value = 0;
	...
	snd_soc_write(codec, ACM8625_REG_00, ACM8625_PAGE_00);
	snd_soc_write(codec, ACM8625_REG_04, reg04_value);
	return 0;
}
			       ...
			       struct snd_soc_dai *codec_dai)
{
	...
	struct snd_soc_codec *codec = acm8625->codec;
...
static int acm8625_snd_suspend(struct snd_soc_codec *codec)
{
	struct acm8625_priv *acm8625 = snd_soc_codec_get_drvdata(codec);
	struct acm8625_platform_data *pdata = acm8625->pdata;

	dev_info(codec->dev, "acm8625_suspend!\n");
	acm8625_set_bias_level(codec, SND_SOC_BIAS_OFF);
...
static int acm8625_reg_init(struct snd_soc_codec *codec)
{
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(acm8625_reset); i++) {
		snd_soc_write(codec, acm8625_reset[i][0],
		acm8625_reset[i][1]);
	};
	usleep_range(10 * 1000, 11 * 1000);

	for (i = 0; i < ARRAY_SIZE(acm8625_init_sequence); i++) {
		snd_soc_write(codec, acm8625_init_sequence[i][0],
...
static int acm8625_snd_resume(struct snd_soc_codec *codec)
{
	int ret;
	struct acm8625_priv *acm8625 = snd_soc_codec_get_drvdata(codec);
	struct acm8625_platform_data *pdata = acm8625->pdata;
...
static int acm8625_probe(struct snd_soc_codec *codec)
{
	int ret;
	struct acm8625_priv *acm8625 = snd_soc_codec_get_drvdata(codec);
	...
	acm8625_set_volume(codec, acm8625->vol);
	snd_soc_add_codec_controls(codec, acm8625_vol_control,
...
static int acm8625_remove(struct snd_soc_codec *codec)
{
	struct acm8625_priv *acm8625 = snd_soc_codec_get_drvdata(codec);
	struct acm8625_platform_data *pdata = acm8625->pdata;
...
static struct snd_soc_codec_driver soc_codec_acm8625 = {
	.probe = acm8625_probe,
			      ...
			      const struct i2c_device_id *id)
{
	...
	ret =
		snd_soc_register_codec(&i2c->dev, &soc_codec_acm8625,
			&acm8625_dai, 1);
	if (ret != 0)
		return -ENOMEM;
