/*
 * ad82120b.c  --  ad82120b ALSA SoC Audio driver
 *
...
struct ad82120b_data {
	...
	struct regulator_bulk_data supplies[AD82120B_NUM_SUPPLIES];
...
static int ad82120b_pd_gpio_set(struct snd_soc_codec *codec, bool enable)
{
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	int ret;
	if (!(gpio_is_valid(ad82120b->pd_gpio))) {
		printk("ad82120b the pd gpio is un-valid\n");
		return 0;
	}
	if (enable) {
		if (!reset_pin_setting) {
			// request amp PD pin control GPIO
			ret = gpio_request(ad82120b->pd_gpio, NULL);
			if (ret < 0) {
				dev_err(codec->dev,
			...
			gpio_direction_output(ad82120b->pd_gpio, 1);
			reset_pin_setting = 1;
			gpio_free(ad82120b->pd_gpio);
			dev_info(codec->dev,
				 "ad82120b pd pin status = %d\n",
				 gpio_get_value(ad82120b->pd_gpio));
		}
	} else {
		if (reset_pin_setting) {
			// request amp PD pin control GPIO
			...
			gpio_direction_output(ad82120b->pd_gpio, 0);
			reset_pin_setting = 0;
			gpio_free(ad82120b->pd_gpio);
			/*gpio_direction_output(ad82120b->pd_gpio, GPIOF_OUT_INIT_LOW); */
			dev_info(codec->dev,
				 "ad82120b pd pin status = %d\n",
				 gpio_get_value(ad82120b->pd_gpio));
		}
			      ...
			      struct snd_soc_dai *dai)
{
	...
	ad82120b_pd_gpio_set(codec, true);	// pull high amp PD pin
	msleep(20);
	printk("setting ad82120b hw params.\n");
	ret = snd_soc_update_bits(codec, AD82120B_STATE_CTRL2_REG,
...
static int ad82120b_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	ad82120b_pd_gpio_set(codec, true);	// pull high amp PD pin
	msleep(20);
	printk("setting ad82120b dai format.\n");
	ret = snd_soc_update_bits(codec, AD82120B_STATE_CTRL1_REG,
				  AD82120B_SAIF_FORMAT_MASK, serial_format);
	if (ret < 0) {
...
#ifdef AD82120B_CHANGE_EQ_MODE_EN
static int ad82120b_change_eq_mode(struct snd_soc_codec *codec, int channel)
{
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	int eq_seg = 0;
	int i = 0;
	...
	for (i = 0; i < 20; i++) {
		// ram addr
		regmap_write(ad82120b->regmap, 0x1d,
			     ad82120b->m_ram_tab[eq_seg][0]);
		// write A1
		regmap_write(ad82120b->regmap, 0x1e,
			     ad82120b->m_ram_tab[eq_seg][1]);
		regmap_write(ad82120b->regmap, 0x1f,
			     ad82120b->m_ram_tab[eq_seg][2]);
		regmap_write(ad82120b->regmap, 0x20,
			     ad82120b->m_ram_tab[eq_seg][3]);
		regmap_write(ad82120b->regmap, 0x21,
			     ad82120b->m_ram_tab[eq_seg][4]);
		eq_seg += 1;
		// write A2
		regmap_write(ad82120b->regmap, 0x22,
			     ad82120b->m_ram_tab[eq_seg][1]);
		regmap_write(ad82120b->regmap, 0x23,
			     ad82120b->m_ram_tab[eq_seg][2]);
		regmap_write(ad82120b->regmap, 0x24,
			     ad82120b->m_ram_tab[eq_seg][3]);
		regmap_write(ad82120b->regmap, 0x25,
			     ad82120b->m_ram_tab[eq_seg][4]);
		eq_seg += 1;
		// write B1
		regmap_write(ad82120b->regmap, 0x26,
			     ad82120b->m_ram_tab[eq_seg][1]);
		regmap_write(ad82120b->regmap, 0x27,
			     ad82120b->m_ram_tab[eq_seg][2]);
		regmap_write(ad82120b->regmap, 0x28,
			     ad82120b->m_ram_tab[eq_seg][3]);
		regmap_write(ad82120b->regmap, 0x29,
			     ad82120b->m_ram_tab[eq_seg][4]);
		eq_seg += 1;
		// write B2
		regmap_write(ad82120b->regmap, 0x2a,
			     ad82120b->m_ram_tab[eq_seg][1]);
		regmap_write(ad82120b->regmap, 0x2b,
			     ad82120b->m_ram_tab[eq_seg][2]);
		regmap_write(ad82120b->regmap, 0x2c,
			     ad82120b->m_ram_tab[eq_seg][3]);
		regmap_write(ad82120b->regmap, 0x2d,
			     ad82120b->m_ram_tab[eq_seg][4]);
		eq_seg += 1;
		// write A0
		regmap_write(ad82120b->regmap, 0x2e,
			     ad82120b->m_ram_tab[eq_seg][1]);
		regmap_write(ad82120b->regmap, 0x2f,
			     ad82120b->m_ram_tab[eq_seg][2]);
		regmap_write(ad82120b->regmap, 0x30,
			     ad82120b->m_ram_tab[eq_seg][3]);
		regmap_write(ad82120b->regmap, 0x31,
			     ad82120b->m_ram_tab[eq_seg][4]);
		if (channel == 1)
			cmd = 0x02;
		else if (channel == 2)
	...
	for (eq_seg = 0x73; eq_seg < 0x9E; eq_seg++) {
		if ((eq_seg >= 0x7E) && (eq_seg <= 0x81))
			continue;
		regmap_write(ad82120b->regmap, CFADDR,
			     ad82120b->m_ram_tab[eq_seg][0]);
		regmap_write(ad82120b->regmap, A1CF1,
			     ad82120b->m_ram_tab[eq_seg][1]);
		regmap_write(ad82120b->regmap, A1CF2,
			     ad82120b->m_ram_tab[eq_seg][2]);
		regmap_write(ad82120b->regmap, A1CF3,
			     ad82120b->m_ram_tab[eq_seg][3]);
		regmap_write(ad82120b->regmap, A1CF4,
			     ad82120b->m_ram_tab[eq_seg][4]);
		if (channel == 1)
			cmd = 0x01;
		else if (channel == 2)
...
static int ad82120b_eq_mode_info(struct snd_kcontrol *kcontrol,
...
static int ad82120b_eq_mode_get(struct snd_kcontrol *kcontrol,
...
static int ad82120b_eq_mode_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	...
	if ((id_reg & 0xf0) != AD82120B_DEVICE_ID)	// amp i2c have not ack ,i2c error
	{
		dev_err(codec->dev,
...
static void ad82120b_fault_check_work(struct work_struct *work)
{
	struct ad82120b_data *ad82120b =
	    container_of(work, struct ad82120b_data,
			 fault_check_work.work);
	struct device *dev = ad82120b->codec->dev;
	...
	if ((ad82120b->CHIP_SYNC == 1) && (ad82120b->CHIP_SYNC_lock == 0)) {
		...
		ret =
		    snd_soc_update_bits(ad82120b->codec,
					AD82120B_STATE_CTRL4_REG,
					AD82120B_CHIP_SYNC_EN,
					AD82120B_CHIP_SYNC_EN);
		msleep(1);
	...
	if ((ad82120b->ASR_DET != 0) && (ad82120b->ASR_DET_lock == 0)) {
		ret =
		    regmap_write(ad82120b->regmap, AD82120B_CLK_DET_CTRL,
				 ad82120b->ASR_DET);
	...
	curr_fault &=
	    AD82120B_OCE | AD82120B_DCDE | AD82120B_OTE | AD82120B_UVE |
	    AD82120B_BSUVE | AD82120B_OVPE;
	/*
	 ...
	 */
	if (!(curr_fault & AD82120B_OCE)
	    && (ad82120b->last_fault & AD82120B_OCE))
		dev_crit(dev, "experienced an over current hardware fault\n");
	if (!(curr_fault & AD82120B_DCDE)
	    && (ad82120b->last_fault & AD82120B_DCDE))
		dev_crit(dev, "experienced an DCD detection fault\n");
	if (!(curr_fault & AD82120B_OTE)
	    && (ad82120b->last_fault & AD82120B_OTE))
		dev_crit(dev, "experienced an over temperature fault\n");
	if (!(curr_fault & AD82120B_UVE)
	    && (ad82120b->last_fault & AD82120B_UVE))
		dev_crit(dev, "experienced an UV fault\n");
	if (!(curr_fault & AD82120B_BSUVE)
	    && (ad82120b->last_fault & AD82120B_BSUVE))
		dev_crit(dev, "experienced an BSUV fault\n");
	if (!(curr_fault & AD82120B_OVPE)
	    && (ad82120b->last_fault & AD82120B_OVPE))
	...
	ad82120b_pd_gpio_set(ad82120b->codec, false);	// pull low amp PD pin
	msleep(20);
	ad82120b_pd_gpio_set(ad82120b->codec, true);	// pull high amp PD pin
out:
	/* Schedule the next fault check at the specified interval */
	schedule_delayed_work(&ad82120b->fault_check_work,
			      msecs_to_jiffies(AD82120B_FAULT_CHECK_INTERVAL));
}
static int ad82120b_reg_ram_init(struct snd_soc_codec *codec)
{
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	int ret;
	int i;
	int reg_data;
	printk("ad82120b i2c address = %p,  %s!\n", codec, __func__);
	ad82120b_pd_gpio_set(codec, true);	// pull high amp PD pin
	msleep(45);
	// software reset amp
	ret = snd_soc_update_bits(codec, AD82120B_STATE_CTRL5_REG,
				  AD82120B_SW_RESET, 0);
	if (ret < 0)
	...
	msleep(20);		// wait 20ms
	/* Set device to mute */
	ret = snd_soc_update_bits(codec, AD82120B_STATE_CTRL3_REG,
	...
	for (i = 0; i < AD82120B_REGISTER_COUNT; i++) {
		...
		if (ad82120b->ch_mode) {
			...
			if (m_reg_tab[i][0] == 0x1A) {
				reg_data = 0x68;
				reg_data |= 0x40;	// mono_en = 1
			}
			if (m_reg_tab[i][0] == 0x5B) {
				reg_data = 0x30;
			}
			if (m_reg_tab[i][0] == 0x5C)	// mono_key = 0x3006
			{
				reg_data = 0x06;
			}
			if (m_reg_tab[i][0] == 0x98)	// mono_key = 0x3006
			{
				reg_data = 0x12;
			}
			if (m_reg_tab[i][0] == 0x9A)	// mono_key = 0x3006
			{
				reg_data = 0x64;
			}
			// set mono end
			...
			if (m_reg_tab[i][0] == 0x5C)	// mono_key = 0x0000
			{
				reg_data = 0x00;
			}
			// set stereo end
		...
		if (m_reg_tab[i][0] == AD82120B_STATE_CTRL4_REG) {
			if ((reg_data & AD82120B_CHIP_SYNC_EN) ==
			    AD82120B_CHIP_SYNC_EN) {
				ad82120b->CHIP_SYNC = 1;
				printk("ad82120b chip sync enable!\n");
			} else
		...
		if (m_reg_tab[i][0] == AD82120B_CLK_DET_CTRL) {
			if ((reg_data & AD82120B_ASR_DET) == AD82120B_ASR_DET) {
				...
				printk
				    ("ad82120b Auto sample rate detection enable!\n");
	...
	for (i = 0; i < AD82120B_RAM1_TABLE_COUNT; i++) {
		if (ad82120b->ch_mode) {
			// set mono
			if (m_ram1_tab[i][0] == 0x73) {
				m_ram1_tab[i][1] = 0x04;
				m_ram1_tab[i][2] = 0x00;
				m_ram1_tab[i][3] = 0x00;
				m_ram1_tab[i][4] = 0x00;
			}
			if (m_ram1_tab[i][0] == 0x74) {
				m_ram1_tab[i][1] = 0x04;
				m_ram1_tab[i][2] = 0x00;
				m_ram1_tab[i][3] = 0x00;
				m_ram1_tab[i][4] = 0x00;
	...
	for (i = 0; i < AD82120B_RAM2_TABLE_COUNT; i++) {
		if (ad82120b->ch_mode) {
			// set mono
			if (m_ram2_tab[i][0] == 0x73) {
				m_ram2_tab[i][1] = 0x04;
				m_ram2_tab[i][2] = 0x00;
				m_ram2_tab[i][3] = 0x00;
				m_ram2_tab[i][4] = 0x00;
			}
			if (m_ram2_tab[i][0] == 0x74) {
				m_ram2_tab[i][1] = 0x04;
				m_ram2_tab[i][2] = 0x00;
				m_ram2_tab[i][3] = 0x00;
				m_ram2_tab[i][4] = 0x00;
...
static int ad82120b_codec_probe(struct snd_soc_codec *codec)
{
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	...
	msleep(10);
	ret = ad82120b_reg_ram_init(codec);
	if (ret < 0)
	...
	if (ad82120b->CHIP_SYNC == 1) {
		/* Set CHIP SYNC to disable */
		ret = snd_soc_update_bits(codec, AD82120B_STATE_CTRL4_REG,
					  AD82120B_CHIP_SYNC_EN, 0);
	...
	if (ret < 0)
		goto error_snd_soc_component_update_bits;
	INIT_DELAYED_WORK(&ad82120b->fault_check_work,
			  ad82120b_fault_check_work);
#ifdef AD82120B_CHANGE_EQ_MODE_EN
	ret = snd_soc_add_codec_controls(codec, ad82120b_eq_mode_control, 1);
	if (ret != 0) {
		printk
		    ("Failed to register ad82120b_eq_mode_control (%d)\n", ret);
	}
#endif
#ifdef AD82120B_REG_RAM_CHECK
	msleep(1000);
	for (i = 0; i < AD82120B_REGISTER_COUNT; i++) {
		...
		//printk("read ad82120b register {addr, data} = {0x%02x, 0x%02x}\n", m_reg_tab[i][0], reg_data);
	}
	for (i = 0; i < AD82120B_RAM1_TABLE_COUNT; i++) {
		regmap_write(ad82120b->regmap, CFADDR, m_ram1_tab[i][0]);	// write ram addr
		regmap_write(ad82120b->regmap, CFUD, 0x04);	// write read a single ram data cmd
		...
		/*printk("read ad82120b ram1 {addr, H8, M8, L8, LL8} = {0x%02x, 0x%02x, 0x%02x, 0x%02x, 0x%02x}\n", \
...
//probe_fail:
	//regulator_bulk_disable(ARRAY_SIZE(ad82120b->supplies),
	//ad82120b->supplies);
	return ret;
}
static int ad82120b_codec_remove(struct snd_soc_codec *codec)
{
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	int ret;
	...
	ad82120b->CHIP_SYNC_lock = 0;
	ad82120b->ASR_DET_lock = 0;
	ret = regulator_bulk_disable(ARRAY_SIZE(ad82120b->supplies),
				     ad82120b->supplies);
	if (ret < 0)
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	if (event & SND_SOC_DAPM_POST_PMU) {
		printk("ad82120b dac event post PMU.\n");
		...
		schedule_delayed_work(&ad82120b->fault_check_work,
				      msecs_to_jiffies
				      (AD82120B_FAULT_CHECK_INTERVAL));
	} else if (event & SND_SOC_DAPM_PRE_PMD) {
		printk("ad82120b dac event pre PMD.\n");
		...
		if (ad82120b->ASR_DET != 0) {
			regmap_write(ad82120b->regmap,
				     AD82120B_CLK_DET_CTRL, 0x10);
		}
		ad82120b->ASR_DET_lock = 0;
		...
		if (ret < 0)
			dev_err(codec->dev,
				"failed to write MUTE register: %d\n", ret);
		msleep(20);
		ad82120b_pd_gpio_set(codec, false);	// pull low amp PD pin
	}
	return 0;
}
static int ad82120b_mute(struct snd_soc_codec *codec, int mute)
{
	int ret;
	if (mute) {
		//mute master volume
...
#ifdef CONFIG_PM
static int ad82120b_suspend(struct snd_soc_codec *codec)
{
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	int ret;
	printk("ad82120b suspend.\n");
	...
	if (ret < 0)
		dev_err(codec->dev, "failed to disable supplies: %d\n", ret);
	ad82120b_pd_gpio_set(codec, false);
	return ret;
}
static int ad82120b_resume(struct snd_soc_codec *codec)
{
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	int ret, i, j;
	char *p = ad82120b->m_reg_tab;
	printk("ad82120b resume.\n");
	ret = regulator_bulk_enable(ARRAY_SIZE(ad82120b->supplies),
				    ad82120b->supplies);
	if (ret < 0) {
		dev_err(codec->dev, "failed to enable supplies: %d\n", ret);
	...
	ad82120b_mute(codec, ad82120b->mute);
	return 0;
...
static int ad82120b_trigger(struct snd_pcm_substream *substream, int cmd,
			    struct snd_soc_dai *codec_dai)
{
	struct ad82120b_data *ad82120b = snd_soc_dai_get_drvdata(codec_dai);
	struct snd_soc_codec *codec = ad82120b->codec;
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
			pr_debug("%s(), stop\n", __func__);
			if (!ad82120b->mute)
				ad82120b_mute(codec, 1);
			break;
		}
	}
	return 0;
}
static int ad82120b_mute_info(struct snd_kcontrol *kcontrol,
			      struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->access =
...
static int ad82120b_mute_locked_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	ad82120b->mute = ucontrol->value.integer.value[0];
	ad82120b_mute(codec, ad82120b->mute);
	return 0;
}
static int ad82120b_mute_locked_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	ucontrol->value.integer.value[0] = ad82120b->mute;
	return 0;
}
static int ad82120b_vol_info(struct snd_kcontrol *kcontrol,
			     struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	uinfo->access =
	    (SNDRV_CTL_ELEM_ACCESS_TLV_READ | SNDRV_CTL_ELEM_ACCESS_READWRITE);
	uinfo->count = 1;
	uinfo->value.integer.min = AD82120B_VOLUME_MIN;
	uinfo->value.integer.max = AD82120B_VOLUME_MAX;
	uinfo->value.integer.step = 1;
	return 0;
}
...
static void ad82120b_set_volume(struct snd_soc_codec *codec, int vol)
{
	unsigned int index;
	u32 volume_hex;
	u8 byte;
	...
	snd_soc_write(codec, AD82120B_VOLUME_CTRL_REG, byte);
}
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	ucontrol->value.integer.value[0] = ad82120b->vol;
	return 0;
}
static int ad82120b_vol_locked_put(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	ad82120b->vol = ucontrol->value.integer.value[0];
	ad82120b_set_volume(codec, ad82120b->vol);
	return 0;
}
static int ad82120b_set_ram1_param(struct snd_kcontrol *kcontrol,
				   ...
				   unsigned int size)
{
	...
	res = copy_from_user(p_string, val, AD82120B_PARAM_COUNT);
	if (res)
		return -EFAULT;
	memcpy(ad82120b->m_ram1_tab, p_string, AD82120B_RAM1_TABLE_COUNT * 5);
	for (i = 0; i < AD82120B_RAM1_TABLE_COUNT; i++) {
		for (j = 0; j < 5; j++)
...
static int ad82120b_get_ram1_param(struct snd_kcontrol *kcontrol,
				   ...
				   unsigned int size)
{
	...
	for (i = 0; i < AD82120B_RAM1_TABLE_COUNT; i++) {
		...
		regmap_read(ad82120b->regmap, A1CF1, &m_ram1_tab[i][1]);
		regmap_read(ad82120b->regmap, A1CF2, &m_ram1_tab[i][2]);
		regmap_read(ad82120b->regmap, A1CF3, &m_ram1_tab[i][3]);
		regmap_read(ad82120b->regmap, A1CF4, &m_ram1_tab[i][4]);
	}
	for (i = 0; i < AD82120B_RAM1_TABLE_COUNT; i++) {
		for (j = 0; j < 5; j++)
			tmp_string[i * 5 + j] = m_ram1_tab[i][j];
	}
	res = copy_to_user(val, p_string, AD82120B_RAM1_TABLE_COUNT * 5);
	if (res)
		return -EFAULT;
	return 0;
}
static int ad82120b_set_ram2_param(struct snd_kcontrol *kcontrol,
				   const unsigned int __user *bytes,
				   unsigned int size)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	...
	if (res)
		return -EFAULT;
	memcpy(ad82120b->m_ram2_tab, p_string, AD82120B_RAM2_TABLE_COUNT * 5);
	for (i = 0; i < AD82120B_RAM2_TABLE_COUNT; i++) {
		for (j = 0; j < 5; j++)
...
static int ad82120b_get_ram2_param(struct snd_kcontrol *kcontrol,
				   unsigned int __user *bytes,
				   unsigned int size)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	...
	for (i = 0; i < AD82120B_RAM2_TABLE_COUNT; i++) {
		regmap_write(ad82120b->regmap, CFADDR, m_ram2_tab[i][0]);
		regmap_write(ad82120b->regmap, CFUD, 0x04);
		regmap_read(ad82120b->regmap, A1CF1, &m_ram2_tab[i][1]);
		regmap_read(ad82120b->regmap, A1CF2, &m_ram2_tab[i][2]);
		regmap_read(ad82120b->regmap, A1CF3, &m_ram2_tab[i][3]);
		regmap_read(ad82120b->regmap, A1CF4, &m_ram2_tab[i][4]);
	}
	for (i = 0; i < AD82120B_RAM2_TABLE_COUNT; i++) {
		for (j = 0; j < 5; j++)
			tmp_string[i * 5 + j] = m_ram2_tab[i][j];
	}
	res = copy_to_user(val, p_string, AD82120B_RAM2_TABLE_COUNT * 5);
	if (res)
		return -EFAULT;
	return 0;
}
static int ad82120b_set_regeister_value(struct snd_kcontrol *kcontrol,
					const unsigned int __user *bytes,
					unsigned int size)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	...
	if (res)
		return -EFAULT;
	memcpy(p, p_string, AD82120B_REGISTER_COUNT * 2);
	for (i = 0; i < AD82120B_REGISTER_COUNT; i++) {
		snd_soc_write(codec, *p, *(p + 1));
		p += 2;
	}
...
static int ad82120b_get_regeister_value(struct snd_kcontrol *kcontrol,
					unsigned int __user *bytes,
					unsigned int size)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
...
static int ad82120b_set_reg_setting_enable(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value
					   *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	ad82120b->reg_setting = ucontrol->value.integer.value[0];
	return 0;
}
static int ad82120b_get_reg_setting_enable(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value
					   *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	bool enable = (bool)ad82120b->reg_setting & 0x1;
	ucontrol->value.integer.value[0] = enable;
	return 0;
}
static int ad82120b_set_EQ_enum(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	ad82120b->eq_enable = ucontrol->value.integer.value[0];
	return 0;
}
static int ad82120b_get_EQ_enum(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	bool enable = (bool)ad82120b->eq_enable & 0x1;
	ucontrol->value.integer.value[0] = enable;
	return 0;
}
static int ad82120b_set_DRC_enum(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	ad82120b->drc_enable = ucontrol->value.integer.value[0];
	return 0;
}
static int ad82120b_get_DRC_enum(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct ad82120b_data *ad82120b = snd_soc_codec_get_drvdata(codec);
	bool enable = (bool)ad82120b->drc_enable & 0x1;
	ucontrol->value.integer.value[0] = enable;
	return 0;
}
static const struct snd_kcontrol_new ad82120b_snd_controls[] = {
	{
	 .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	 .name = "Master Volume",
	 .info = ad82120b_vol_info,
	 .get = ad82120b_vol_locked_get,
	 .put = ad82120b_vol_locked_put,
	  },
	{
	 .iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	 .name = "Maser Volume Mute",
	 .info = ad82120b_mute_info,
	 .get = ad82120b_mute_locked_get,
	 .put = ad82120b_mute_locked_put,
	  },
	SOC_SINGLE_BOOL_EXT("Register setting enable", 0,
			    ad82120b_get_reg_setting_enable,
			    ad82120b_set_reg_setting_enable),
	SOC_SINGLE_BOOL_EXT("Set EQ Enable", 0,
			    ad82120b_get_EQ_enum, ad82120b_set_EQ_enum),
	SOC_SINGLE_BOOL_EXT("Set DRC Enable", 0,
			    ad82120b_get_DRC_enum, ad82120b_set_DRC_enum),
	SND_SOC_BYTES_TLV("Register value", AD82120B_PARAM_COUNT,
			  ad82120b_get_regeister_value,
			  ad82120b_set_regeister_value),
	SND_SOC_BYTES_TLV("EQ table", AD82120B_PARAM_COUNT,
			  ad82120b_get_ram1_param,
			  ad82120b_set_ram1_param),
	SND_SOC_BYTES_TLV("DRC table", AD82120B_PARAM_COUNT,
			  ad82120b_get_ram2_param,
			  ad82120b_set_ram2_param),
};
...
static const struct snd_soc_codec_driver soc_component_dev_ad82120b = {
	...
	.component_driver = {
			     ...
			     .num_dapm_widgets =
			     ARRAY_SIZE(ad82120b_dapm_widgets),
...
/* Formats supported by AD82120B driver */
/*
#define AD82120B_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S18_LE |\
...
static int ad82120b_parse_dt(struct ad82120b_data *ad82120b, struct device *dev)
{
	...
	ad82120b->pd_gpio = of_get_named_gpio(np, "pd-gpio", 0);
	dev_info(dev, "ad82120b parse dts,the pd-gpio is %d\n",
		 ad82120b->pd_gpio);
	if (!gpio_is_valid(ad82120b->pd_gpio)) {
		dev_err(dev, "get invalid extamp-pd-gpio %d\n",
			ad82120b->pd_gpio);
		ret = -EINVAL;
	}
	of_property_read_u32(np, "channel_mode", &ch_mode);
			  ...
			  const struct i2c_device_id *id)
{
	...
	for (i = 0; i < ARRAY_SIZE(data->supplies); i++)
		data->supplies[i].supply = ad82120b_supply_names[i];
	...
	data->m_reg_tab =
	    devm_kzalloc(&client->dev,
			 sizeof(char) * AD82120B_PARAM_COUNT, GFP_KERNEL);
	if (!data->m_reg_tab)
		return -ENOMEM;
	data->m_ram1_tab =
	    devm_kzalloc(&client->dev,
...
static void ad82120b_shutdown(struct i2c_client *client)
{
	struct ad82120b_data *ad82120b = i2c_get_clientdata(client);
	int ret;
	if (!ad82120b)
		return;
	if (gpio_is_valid(ad82120b->pd_gpio) && reset_pin_setting) {
		// request amp PD pin control GPIO
		ret = gpio_request(ad82120b->pd_gpio, NULL);
		if (ret < 0)
